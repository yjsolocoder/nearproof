...
class StreamCommitReceiptFrontierFieldContractTest(unittest.TestCase):
    ...
    def test_version_contract(self):
        for bad in (True, "1", 1.0, None, b"1"):
            with self.assertRaises(TypeError, msg=repr(bad)):
                StreamCommitReceiptFrontier(
                    bad, 1, SFRONTIER_1.to_bytes(), CDIGEST_1, ZERO
                )
        for bad in (0, 2):
    ...
    def test_sequence_contract(self):
        for bad in (True, "1", 1.0, None):
            with self.assertRaises(TypeError, msg=repr(bad)):
                StreamCommitReceiptFrontier(
                    1, bad, SFRONTIER_1.to_bytes(), CDIGEST_1, ZERO
                )
        ...
        self.assertEqual(
            StreamCommitReceiptFrontier(
                1, U64_MAX, SFRONTIER_1.to_bytes(), CDIGEST_1, ZERO
            ).sequence,
            U64_MAX,
        )

    def test_end_contract(self):
        for bad in (1, "x", None, bytearray(SFRONTIER_1.to_bytes())):
            with self.assertRaises(TypeError, msg=repr(bad)):
                StreamCommitReceiptFrontier(1, 1, bad, CDIGEST_1, ZERO)
        # end must be the canonical non-empty StreamReceiptFrontier
        # encoding: not empty, not junk, and not a range-batch receipt
        # frontier.
        for bad in (b"", b"junk", BFRONTIER_1.to_bytes()):
            with self.assertRaises(ValueError, msg=repr(bad)):
                StreamCommitReceiptFrontier(1, 1, bad, CDIGEST_1, ZERO)
...
class StreamCommitReceiptFrontierEncodingTest(unittest.TestCase):
    def test_canonical_encoding_shape(self):
        ...
        outer = json.loads(CFRONTIER_1.to_bytes())
        self.assertEqual(
            outer,
            [
                1,
    ...
    def test_from_bytes_does_not_verify_mac(self):
        ...
        parsed = StreamCommitReceiptFrontier.from_bytes(
            tampered.to_bytes()
        )
...
class StreamCommitReceiptAuditorConstructionTest(unittest.TestCase):
    ...
    def test_checkpoint_nested_stream_frontier_mac_verified(self):
...
class StreamCommitReceiptAuditTest(unittest.TestCase):
    ...
    def test_mixed_object_and_bytes_forms(self):
        auditor = StreamCommitReceiptAuditor(KEY)
        auditor.audit(CRECEIPT_1.to_bytes(), SBATCH_1)
        self.assertEqual(auditor.state, CFRONTIER_1)
        auditor.audit(CRECEIPT_2, SBATCH_2.to_bytes())
        self.assertEqual(auditor.state, CFRONTIER_2)
    ...
    def test_restart_from_checkpoint(self):
        ...
        restored = StreamCommitReceiptAuditor(KEY, checkpoint=blob)
        restored.audit(CRECEIPT_2, SBATCH_2)
        self.assertEqual(restored.state, CFRONTIER_2)

    def test_first_commit_must_start_empty(self):
        # A commit receipt continuing from SFRONTIER_1 cannot be the
        # first commit of a fresh ledger.
        auditor = StreamCommitReceiptAuditor(KEY)
        with self.assertRaises(ValueError):
            auditor.audit(CRECEIPT_2, SBATCH_2)
        self.assertIsNone(auditor.state)

    def test_replayed_last_receipt_rejected_and_unchanged(self):
        auditor = StreamCommitReceiptAuditor(KEY)
        auditor.audit(CRECEIPT_1, SBATCH_1)
    ...
    def test_old_receipt_and_old_fork_rejected(self):
        ...
        auditor.audit(CRECEIPT_2, SBATCH_2)
        before = auditor.state
        # An old receipt replayed after more progress.
        with self.assertRaises(ValueError):
            auditor.audit(CRECEIPT_1, SBATCH_1)
        # A fork that honestly covers the whole history but starts
        # empty again (the genesis full batch) is an old fork.
        with self.assertRaises(ValueError):
            auditor.audit(CRECEIPT_FULL, SBATCH_FULL)
        self.assertIs(auditor.state, before)

    def test_genesis_full_batch_then_continuation_gap_rejected(self):
        auditor = StreamCommitReceiptAuditor(KEY)
        auditor.audit(CRECEIPT_FULL, SBATCH_FULL)
        self.assertEqual(auditor.state.sequence, 1)
        self.assertEqual(auditor.state.end, SFRONTIER_2.to_bytes())
        # CRECEIPT_2 starts at SFRONTIER_1, not SFRONTIER_2.
        with self.assertRaises(ValueError):
            auditor.audit(CRECEIPT_2, SBATCH_2)
    ...
    def test_wrong_argument_type(self):
        auditor = StreamCommitReceiptAuditor(KEY)
        for bad in (
            1, "x", None, [CRECEIPT_1], (CRECEIPT_1,), object()
        ):
            with self.assertRaises(TypeError, msg=repr(bad)):
    ...
    def test_malformed_bytes_is_value_error(self):
        auditor = StreamCommitReceiptAuditor(KEY)
        for bad in (b"junk", b"[1,2,3]", b"\xff"):
            with self.assertRaises(ValueError, msg=repr(bad)):
                auditor.audit(bad, SBATCH_1)
            with self.assertRaises(ValueError, msg=repr(bad)):
                auditor.audit(CRECEIPT_1, bad)
    ...
    def test_tampered_receipt_rejected(self):
        ...
        with self.assertRaises(ValueError):
            auditor.audit(forged, SBATCH_1)
        tampered = dataclasses.replace(CRECEIPT_1, mac=ZERO)
        with self.assertRaises(ValueError):
            auditor.audit(tampered, SBATCH_1)
        self.assertIsNone(auditor.state)
    ...
    def test_wrong_key_rejected(self):
        with self.assertRaises(ValueError):
            StreamCommitReceiptAuditor(OTHER_KEY).audit(
                CRECEIPT_1, SBATCH_1
            )
    ...
    def test_sequence_overflow_changes_no_state(self):
        maxed = commit_frontier_for(
            U64_MAX, SFRONTIER_1.to_bytes(), CDIGEST_1
        )
        auditor = StreamCommitReceiptAuditor(KEY, checkpoint=maxed)
        before = auditor.state
        # CRECEIPT_2 links exactly from SFRONTIER_1, so the link check
        # passes and only the u64 overflow fails.
        with self.assertRaises(ValueError):
            auditor.audit(CRECEIPT_2, SBATCH_2)
        self.assertIs(auditor.state, before)
    ...
    def test_checkpoint_export_round_trip(self):
        auditor = StreamCommitReceiptAuditor(KEY)
        self.assertIsNone(auditor.checkpoint)
        auditor.audit(CRECEIPT_1, SBATCH_1)
