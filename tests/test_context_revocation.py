...
def make_bound(trip=0.5, context=CONTEXT, verifier_key=KEY, prover_key=KEY):
...
class ContextRevocationContractTest(unittest.TestCase):
    ...
    def test_valid_construction_in_order(self):
    ...
    def test_revoked_at_stored_as_float(self):
        ...
        self.assertIsInstance(revocation.revoked_at, float)
    ...
    def test_context_shape_errors_raise_type_error(self):
    ...
    def test_mac_shape_errors_raise_type_error(self):
        for bad in ("00" * 32, None, 32):
            with self.assertRaises(TypeError, msg=repr(bad)):
...
class RevokeContextTest(unittest.TestCase):
    ...
    def test_non_bytes_context_raises_type_error(self):
        for bad in ("c" * 32, None, 32):
            with self.assertRaises(TypeError, msg=repr(bad)):
                revoke_context(bad, 3.0, KEY)

    def test_wrong_length_context_rejected(self):
        for bad in (b"", b"c" * 31, b"c" * 33):
            with self.assertRaises(ValueError, msg=repr(bad)):
                revoke_context(bad, 3.0, KEY)
...
class ContextRevocationSerializationTest(unittest.TestCase):
    ...
    def test_from_bytes_rejects_bad_context_encoding(self):
        for bad in ("63" * 31, "63" * 33, "0G" * 32, "AB" * 32, 123, None):
            with self.assertRaises(ValueError, msg=repr(bad)):
    ...
    def test_from_bytes_rejects_integer_revoked_at_spelling(self):
        # revoked_at is stored as a float, so only the float spelling is
...
class AuditBoundPolicyContextRevocationsTest(unittest.TestCase):
    ...
    def test_matching_context_at_end_rejected(self):
    ...
    def test_matching_context_after_end_rejected(self):
    ...
    def test_non_matching_context_ignored(self):
        revocation = revoke_context(OTHER_CONTEXT, self.end + 1.0, KEY)
        measurement = audit_bound_policy(
            self.record, KEY, now=self.end + 2.0, revocations=[revocation]
        )
        self.assertIsInstance(measurement, Measurement)

    def test_accepts_context_revocation_bytes(self):
        revocation = revoke_context(CONTEXT, self.end - 1.0, KEY)
        measurement = audit_bound_policy(
            self.record, KEY, now=self.end, revocations=[revocation.to_bytes()]
        )
        self.assertIsInstance(measurement, Measurement)

    def test_mixed_objects_and_bytes_and_kinds(self):
        round_revocation = revoke_bound(self.record, 0.0, KEY).to_bytes()
        context_revocation = revoke_context(CONTEXT, 0.0, KEY)
        measurement = audit_bound_policy(
            self.record, KEY, now=self.end,
            revocations=[round_revocation, context_revocation],
        )
        self.assertIsInstance(measurement, Measurement)
    ...
    def test_same_context_as_round_revocation_is_not_a_duplicate(self):
        # The two kinds track different identities; one of each is fine.
        round_revocation = revoke_bound(self.record, 0.0, KEY)
        context_revocation = revoke_context(CONTEXT, 0.0, KEY)
        measurement = audit_bound_policy(
            self.record, KEY, now=self.end,
            revocations=[round_revocation, context_revocation],
    ...
    def test_future_context_revocation_rejected_even_without_match(self):
    ...
    def test_wrong_key_rejected(self):
        revocation = revoke_context(CONTEXT, self.end - 1.0, OTHER_KEY)
        with self.assertRaises(ValueError):
            audit_bound_policy(
                self.record, KEY, now=self.end, revocations=[revocation]
            )

    def test_tampered_context_revocation_rejected(self):
        tampered = dataclasses.replace(
            revoke_context(CONTEXT, self.end - 1.0, KEY), revoked_at=self.end
        )
        with self.assertRaises(ValueError):
            audit_bound_policy(
    ...
    def test_tampered_context_revocation_bytes_rejected(self):
        blob = revoke_context(CONTEXT, 1.0, KEY).to_bytes().replace(
    ...
    def test_now_contract_with_context_revocations(self):
        revocation = revoke_context(CONTEXT, 0.0, KEY)
    ...
    def test_context_revocation_composes_with_max_age(self):
        revocation = revoke_context(CONTEXT, self.end - 1.0, KEY)
        measurement = audit_bound_policy(
