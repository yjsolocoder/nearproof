...
ReceiptStream / SpanReceiptBundle / SPEED_OF_LIGHT_MPS / StreamReceipt /
...
class RangeAuditor:
    ...
    :meth:`audit_bundle` atomically commits one whole-journey
    :class:`SpanReceiptBundle` packing only range receipts (the attested
    commit ranges are not carried): under the same lock shared with
    :meth:`audit`, :meth:`audit_batch`, :meth:`audit_batch_record` and
    :meth:`commit_batch`, the bundle's ``NPBJ35`` signature and every
    MAC layer of both endpoint frontiers are recomputed before anything
    is replayed, the bundle ``start`` must equal the current checkpoint
    byte for byte (an empty ledger accepts only the empty start), every
    carried receipt is then replayed in order on a temporary frontier
    exactly like :meth:`audit` advances it and the read-only
    :attr:`state` is replaced once, only when the replayed final
    frontier equals the bundle's declared ``end`` byte for byte. No
    whole-bundle commit credential is minted — a successful commit only
    ...
    def audit_bundle(self, x: object) -> "RangeAuditor":
        """Atomically commit one whole-journey
        :class:`SpanReceiptBundle` and advance the ledger to its ``end``
        frontier.

        ``x`` must be a :class:`SpanReceiptBundle` or its canonical
        :meth:`SpanReceiptBundle.to_bytes` encoding — any other type
        raises :class:`TypeError`; every other contract violation (an
        empty or wrong key, non-canonical bytes, a wrong
        ``NPBJ35`` signature, a MAC mismatch on either endpoint
        :class:`RangeFrontier`, a tampered or broken carried chain, a
        start that does not match the current checkpoint, a replay of an
        already committed bundle, or a u64 overflow) raises
        :class:`ValueError`. Everything runs under the auditor lock
        shared with :meth:`audit`, :meth:`audit_batch`,
        :meth:`audit_batch_record` and :meth:`commit_batch`, in this
        order:

        1. the bundle is parsed per its canonical contract and its
           ``NPBJ35`` signature recomputed as
           ``HMAC-SHA256(key, b"NPBJ35" + C)`` and compared in constant
           time;
        2. the non-empty ``start`` frontier and the ``end`` frontier are
           each checked at five MAC layers exactly as
           :func:`_verify_range_frontier_macs` recomputes them, and no
           replay begins until both endpoints pass;
        3. the bundle ``start`` must equal the current checkpoint byte
           for byte: an empty ledger accepts only ``start == b""`` and a
           non-empty ledger demands ``start == state.to_bytes()``, which
           rejects an old fork and the same bundle replayed alike;
        4. the carried receipts are replayed in order on a temporary
           frontier only — each receipt's ``NPBJ12`` MAC is re-verified,
           its ``start`` must continue byte for byte from the temporary
           frontier ``end``, the u64 sequence must not overflow and the
           digest-chain head advances as
           ``SHA256(b"NPBJ14" + d + u64be(n) + R)``;
        5. only when every receipt verifies and the temporary final
           frontier's canonical bytes equal the bundle ``end`` byte for
           byte is the read-only :attr:`state` replaced with the
           temporary frontier.

        Any failure raises :class:`ValueError` and leaves the
        checkpoint untouched; re-submitting an already committed bundle
        is rejected because its ``start`` no longer matches the advanced
        checkpoint, and concurrent calls linearize in lock-acquisition
        order so a committed journey is never lost or partially
        applied. No whole-bundle commit credential is minted; a
        successful commit only advances the frontier. Returns the
        auditor itself.
        """
        with self._lock:
            bundle = _coerce_span_receipt_bundle(x, "x")
            # Independently recompute the NPBJ35 bundle signature and
            # every MAC layer of both endpoint frontiers before any
            # replay touches the temporary chain; every comparison is
            # constant time and a mismatch raises ValueError.
            if not hmac.compare_digest(
            ...
            if bundle.start:
                ...
                _verify_range_frontier_macs(self._key, start_frontier)
            _verify_range_frontier_macs(
                self._key, RangeFrontier.from_bytes(bundle.end)
            )
            # The bundle must extend exactly the current checkpoint: the
            # empty start only for an auditor with no receipt yet,
            # otherwise the current state encoding byte for byte. This
            # is what rejects an old fork and the same bundle replayed.
            current = (
                b""
                if self._frontier is None
                else self._frontier.to_bytes()
            ...
            if bundle.start != current:
                raise ValueError(
                    "span receipt bundle start does not match the"
                    " current range frontier"
                )
            # Replay the whole carried receipt chain on a temporary
            # frontier seeded with the matching starting point. The
            # live frontier is read but never mutated until it adopts
            # the temporary frontier below.
...
def _require_span_receipt_bundle_frontier(
    ...
    ``value`` is already known to be ``bytes``; when ``allow_empty``
    holds, ``b""`` (the chain starts with no range receipt at all) is
    also accepted. :meth:`RangeFrontier.from_bytes` enforces the full
    contract including its canonical re-encoding check, and every
    violation — including the field-shape :class:`TypeError` a malformed
    inner document would otherwise surface — raises
    :class:`ValueError`."""
...
def _require_span_receipt_bundle_receipt(value: bytes) -> None:
    ...
    try:
        ...
        raise ValueError(
            "span receipt bundle receipts items must be the canonical"
            " RangeReceipt encoding"
        ) from error


def _span_receipt_bundle_content(bundle: "SpanReceiptBundle") -> list:
...
class SpanReceiptBundle:
    ...
    :class:`RangeReceipt` range receipts as a single transferable
    journey, so a third party can verify the whole journey and book it
    atomically.

    ``version`` is always ``1``. ``start`` is the canonical
    :meth:`RangeFrontier.to_bytes` encoding of the range frontier the
    chain starts from, or ``b""`` when the chain starts from the empty
    ledger (sequence zero, no range receipt at all, the digest chain
    rooted at ``d0``). ``receipts`` is a non-empty ordered tuple of
    canonical :meth:`RangeReceipt.to_bytes` bytes — the range receipts to
    replay in order; the bundle carries the receipt bodies only, the
    attested commit ranges are not carried or replayed with it. ``end``
    is the canonical non-empty :meth:`RangeFrontier.to_bytes` encoding of
    the range frontier the chain ends at. ``signature`` is exactly 32
    bytes — ``HMAC-SHA256(key, b"NPBJ35" + C)`` where ``C`` is the
    canonical compact encoding of the first four fields (the version,
    the lowercase-hex start, the array of lowercase-hex receipt
    encodings and the lowercase-hex end, without ``signature``), the
    prefix and ``C`` concatenated directly with no separator or length
    prefix. Instances are frozen, constructed positionally in field
    order and compare equal by their fields. A field of the wrong type
    raises :class:`TypeError`; every other contract violation raises
    :class:`ValueError`. No key material is stored.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.start, bytes):
            raise TypeError("span receipt bundle start must be bytes")
        ...
        for receipt in self.receipts:
            if not isinstance(receipt, bytes):
                raise TypeError(
                    "span receipt bundle receipts items must be bytes"
                )
            _require_span_receipt_bundle_receipt(receipt)
        if not isinstance(self.end, bytes):
            raise TypeError("span receipt bundle end must be bytes")
        _require_span_receipt_bundle_frontier(
            self.end, "end", allow_empty=False
        )
        if not isinstance(self.signature, bytes):
    ...
    def to_bytes(self) -> bytes:
        ...
        of the lowercase-hex canonical receipt encodings, no whitespace,
        no length prefix."""
        return _encode_payload(
            _span_receipt_bundle_content(self) + [self.signature.hex()]
        )
    ...
    def from_bytes(cls, data: bytes) -> "SpanReceiptBundle":
        ...
        order, ``version == 1``, ``start`` a lowercase hex string that is
        empty or decodes to the canonical :class:`RangeFrontier`
        encoding, ``receipts`` a non-empty array of lowercase hex
        strings each decoding to the canonical :class:`RangeReceipt`
        encoding, ``end`` a lowercase hex string decoding to the
        canonical non-empty :class:`RangeFrontier` encoding and
        ...
        verified here — neither the bundle signature nor any MAC of the
        endpoint frontiers or the carried receipts; pass the record to
        :func:`audit_span_receipt_bundle` with the shared key for that.
        """
        if not isinstance(data, bytes):
            raise TypeError("span receipt bundle data must be bytes")
        try:
            outer = json.loads(data)
        except ValueError as error:
            raise ValueError(
        ...
        if raw_version != 1:
            raise ValueError("span receipt bundle version must be 1")
        start = _parse_bit_map_hex(
            raw_start, "span receipt bundle start"
        )
        _require_span_receipt_bundle_frontier(
            start, "start", allow_empty=True
        )
        ...
        receipts = tuple(
            _parse_bit_map_hex(
                raw_receipt, "span receipt bundle receipt"
            )
            for raw_receipt in raw_receipts
        )
        for receipt in receipts:
...
def _coerce_span_receipt_bundle(
    ...
    splitting the TypeError/ValueError contract exactly as the public
    verifiers do: the wrong kind of argument raises :class:`TypeError`,
...
def _replay_span_receipt_bundle(
    ...
    """Replay a bundle's range-receipt chain onto a tentative ledger
    frontier.

    ``frontier`` is the parsed and MAC-verified starting frontier or
    ``None`` for the empty ledger (sequence zero, the digest chain
    rooted at ``d0``). Each carried encoding is re-parsed and its
    ``NPBJ12`` receipt MAC recomputed and compared in constant time —
    the bundle carries the receipt bodies only, so the attested commit
    ranges are not replayed — and the receipt ``start`` must continue
    byte for byte from the tentative frontier ``end`` before the u64
    sequence, the digest-chain head, the end frontier and the frontier
    MAC advance. The replay touches no shared state: the caller adopts
    the returned tentative frontier only after its own endpoint
    comparison, and every failure raises :class:`ValueError`.
    """
    for encoding in receipt_encodings:
        receipt = RangeReceipt.from_bytes(encoding)
        if not hmac.compare_digest(
            _range_receipt_mac(key, receipt), receipt.mac
        ):
            raise ValueError(
                "span receipt bundle receipt mac does not match the key"
            )
        current = b"" if frontier is None else frontier.end
        if receipt.start != current:
            raise ValueError(
...
def seal_span_receipt_bundle(
    ...
    """Audit a contiguous :class:`RangeReceipt` chain against the range
    ledger and pack it as a transferable whole-journey
    :class:`SpanReceiptBundle`.

    ``receipts`` must be a non-empty ordered iterable whose items are
    each a :class:`RangeReceipt` or its canonical
    :meth:`RangeReceipt.to_bytes` encoding, and ``key`` the non-empty
    shared ``bytes`` key. ``start`` is keyword-only: ``None`` (the
    default) starts the chain from the empty ledger — sequence zero, no
    range receipt at all, the digest chain rooted at ``d0`` — otherwise
    it must be a :class:`RangeFrontier` or its canonical
    :meth:`RangeFrontier.to_bytes` encoding. A non-iterable
    ``receipts``, a non-``bytes`` ``key``, a wrong-typed item or a
    wrong-kind ``start`` raises :class:`TypeError`; an empty sequence,
    an empty key, a malformed or non-canonical encoding, a MAC
    mismatch, a failed receipt signature check, a sequence overflow or
    a broken chain raises :class:`ValueError`. The whole chain is
    verified first, exactly per ledger semantics — each receipt's
    ``NPBJ12`` MAC recomputed and compared in constant time, each
    receipt ``start`` linked byte for byte to the previous frontier
    ``end`` and the sequence and digest-chain head advanced exactly as
    :class:`RangeAuditor` advances them, starting from the supplied
    frontier — and only on success is the bundle produced: ``start``
    carries the starting point (``b""`` when ``start`` is ``None``,
    else the frontier's canonical encoding), ``receipts`` the canonical
    encoding of every receipt in order and ``end`` the canonical
    encoding of the final range frontier, signed as
    ``HMAC-SHA256(key, b"NPBJ35" + C)``. Sealing touches no auditor
    state.
    """
    try:
        items = list(receipts)  # type: ignore[arg-type]
        ...
        raise TypeError(
            "receipts must be an iterable of RangeReceipt instances or"
            " their canonical bytes"
        ) from None
    # The constructor validates the key (non-empty bytes) and the
    # keyword-only start frontier (a RangeFrontier, its canonical bytes
    # or None), parsing and MAC-checking a supplied frontier at every
    # layer.
    auditor = RangeAuditor(key, checkpoint=start)
    coerced = [
        _coerce_range_receipt(item, "receipts items") for item in items
    ]
    if not coerced:
        raise ValueError("receipts must be a non-empty sequence")
    final = _replay_span_receipt_bundle(
    ...
    if start is None:
        ...
        # Canonical RangeFrontier bytes, already validated by the
        # auditor constructor.
        start_bytes = start  # type: ignore[assignment]
    bundle = SpanReceiptBundle(
        version=1,
    ...
    return replace(
        bundle, signature=_span_receipt_bundle_signature(key, bundle)
    )
...
def audit_span_receipt_bundle(
    ...
    """Re-verify a :class:`SpanReceiptBundle` against ``key`` and replay
    the whole journey to its end frontier.

    ``x`` must be a :class:`SpanReceiptBundle` or its canonical
    :meth:`SpanReceiptBundle.to_bytes` encoding and ``key`` the
    non-empty shared ``bytes`` key — a wrong-typed argument raises
    :class:`TypeError`, every other contract violation raises
    :class:`ValueError`. The bundle signature is recomputed as
    ``HMAC-SHA256(key, b"NPBJ35" + C)`` and compared in constant time;
    then both endpoint frontiers are checked at five MAC layers each
    exactly as :func:`_verify_range_frontier_macs` recomputes them, and
    finally the carried receipts are replayed one by one — each
    receipt's ``NPBJ12`` MAC recomputed and compared in constant time,
    each receipt ``start`` continuing byte for byte from the previous
    frontier ``end`` with the first linking to the bundle ``start``, the
    sequence and digest-chain head advancing exactly as
    :class:`RangeAuditor` advances them — starting from the empty
    ledger when ``start`` is empty and from the carried starting
    frontier otherwise; the replayed final frontier must equal the
    bundle's ``end`` byte for byte. Auditing is a pure check: it touches
    no auditor state and returns no partial result — on success the
    frozen :class:`RangeFrontier` the chain ends at is returned.
    """
    bundle = _coerce_span_receipt_bundle(x, "x")
    if not isinstance(key, bytes):
        raise TypeError("key must be bytes")
