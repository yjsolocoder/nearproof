...
# Domain separation prefix for the history-journal bundle MAC.
...
def _require_bit_map_history_journal_bundle_state(value: bytes, name: str) -> None:
    """Enforce that a bundle state field is the canonical non-empty
    :class:`BitMapHistoryJournalState` encoding. ``value`` is already known
    to be ``bytes``; every violation — including the field-shape
    :class:`TypeError` a malformed inner document would otherwise surface —
    raises :class:`ValueError`."""
    try:
...
def _require_bit_map_history_journal_bundle_evidence(value: bytes) -> None:
    """Enforce that a bundle evidence item is the canonical
    :class:`BitMapHistoryEvidence` encoding. ``value`` is already known to
    be ``bytes``; every violation raises :class:`ValueError`."""
    try:
...
class BitMapHistoryJournalBundle:
    """A key-MAC'd attestation of one contiguous journal segment chain.

    ``version`` is always ``1``. ``start`` is the empty bytes — the chain
    starts from no journal state at all — or the canonical
    :meth:`BitMapHistoryJournalState.to_bytes` encoding of the state the
    chain starts from. ``evidences`` is a non-empty ordered tuple of the
    canonical :meth:`BitMapHistoryEvidence.to_bytes` encodings, one per
    segment in chain order. ``end`` is the canonical
    :meth:`BitMapHistoryJournalState.to_bytes` encoding of the journal
    state the chain finishes at. ``mac`` is exactly 32 bytes —
    ...
    ``start``, the array of lowercase-hex evidence encodings and the
    ...
    def __post_init__(self) -> None:
        ...
        if self.version != 1:
            raise ValueError("bit map history journal bundle version must be 1")
        if not isinstance(self.start, bytes):
            raise TypeError("bit map history journal bundle start must be bytes")
        if self.start:
            _require_bit_map_history_journal_bundle_state(self.start, "start")
        ...
        _require_bit_map_history_journal_bundle_state(self.end, "end")
        if not isinstance(self.mac, bytes):
    ...
    def to_bytes(self) -> bytes:
        ...
        ``mac`` are lowercase hex and ``evidences`` is the array of
        lowercase-hex evidence encodings, no whitespace, no length prefix."""
        return _encode_payload(
    ...
    def from_bytes(cls, data: bytes) -> "BitMapHistoryJournalBundle":
        ...
        decodes to the canonical :class:`BitMapHistoryJournalState`
        encoding, ``evidences`` a non-empty array of lowercase hex strings
        each decoding to the canonical :class:`BitMapHistoryEvidence`
        ...
        is not verified here — pass the record to
        :func:`audit_map_history_journal_bundle` with the shared key for
        that.
        """
        if not isinstance(data, bytes):
...
def seal_map_history_journal_bundle(
    ...
    """Audit a contiguous journal segment chain and seal it as a bundle.

    ``evidences`` must be a non-empty iterable of
    :class:`BitMapHistoryEvidence` instances or their canonical
    :meth:`BitMapHistoryEvidence.to_bytes` encodings — a non-iterable or a
    wrong-typed item raises :class:`TypeError`, an empty sequence or a
    malformed item raises :class:`ValueError`. ``key`` must be non-empty
    ``bytes`` — a non-bytes value raises :class:`TypeError`, an empty value
    :class:`ValueError`. ``state`` is keyword-only: ``None`` (the default)
    starts the chain from no journal state at all, so the bundle's ``start``
    is the empty bytes; otherwise it must be a
    :class:`BitMapHistoryJournalState` or its canonical
    :meth:`BitMapHistoryJournalState.to_bytes` encoding (any other type
    raises :class:`TypeError`), checked at both MAC layers exactly as
    :class:`BitMapHistoryJournalAuditor` checks it. Every segment is
    replayed in order through a :class:`BitMapHistoryJournalAuditor` — a
    MAC mismatch, a failed carried update or a broken chain raises
    :class:`ValueError` — and only on success is the bundle produced: the
    canonical evidence encodings in chain order, ``end`` the canonical
    encoding of the final journal state and ``mac`` computed as
    ``HMAC-SHA256(key, b"NPBJ3" + C)``. Sealing touches no auditor state
    beyond the throwaway replay.
    """
    try:
    ...
    if not items:
        raise ValueError("evidences must be a non-empty iterable")
    if not isinstance(key, bytes):
        raise TypeError("key must be bytes")
    if not key:
        raise ValueError("key must be non-empty")
    ...
    final = auditor.state
    assert final is not None  # guaranteed: items is non-empty
    if state is None:
        start = b""
    elif isinstance(state, BitMapHistoryJournalState):
        start = state.to_bytes()
    else:
        # Canonical BitMapHistoryJournalState bytes, already validated.
        start = state  # type: ignore[assignment]
    candidate = BitMapHistoryJournalBundle(
        version=1,
        start=start,
    ...
    return replace(
        candidate,
        mac=_bit_map_history_journal_bundle_mac(key, candidate),
    )
...
def audit_map_history_journal_bundle(
    ...
    the carried ``start`` (when non-empty) and ``end`` states are each
    checked at both MAC layers — the checkpoint table's ``NPBL1`` MAC and
    the state's own ``NPBJ1`` MAC — exactly as
    :class:`BitMapHistoryJournalAuditor` checks them. Finally the carried
    segments are replayed in order through a fresh auditor, starting from
    no journal state when ``start`` is empty and from the carried starting
    state otherwise, and the recomputed final state must equal the bundle's
    ``end`` byte for byte. Auditing is a pure check: it touches no auditor
    state beyond the throwaway replay and returns no partial result — on
    success the frozen :class:`BitMapHistoryJournalState` of the final
    journal is returned.
    """
    ...
    # Both MAC layers of the carried start and end states, always
    # recomputed before the replay is consulted.
    if bundle.start:
        BitMapHistoryJournalAuditor(key, state=bundle.start)
    BitMapHistoryJournalAuditor(key, state=bundle.end)
    ...
    for evidence in bundle.evidences:
        auditor.audit(evidence)
    final = auditor.state
    assert final is not None  # guaranteed: evidences is non-empty
