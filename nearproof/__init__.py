...
ReceiptStream / SPEED_OF_LIGHT_MPS / StreamCommitReceiptAuditor /
StreamCommitReceiptFrontier / StreamReceipt /
...
def _stream_commit_receipt_frontier_next_digest(
    ...
    """One stream-commit-receipt digest-chain step:
    ``d' = SHA256(b"NPBJ26" + d + u64be(n) + R)``.
...
class StreamCommitReceiptFrontier:
    """A key-MAC'd, digest-chained frontier over the audited
    :class:`StreamReceiptBatchCommitReceipt` commit stream.

    ``version`` is always ``1``; ``sequence`` a non-bool unsigned 64-bit
    integer (the number of audited
    :class:`StreamReceiptBatchCommitReceipt` commits); ``end`` the
    canonical non-empty :meth:`StreamReceiptFrontier.to_bytes` encoding
    of the stream-receipt frontier the audited batch-commit chain
    currently ends at; ``digest`` exactly 32 bytes — the head of a hash
    chain with ``d0`` fixed at 32 zero bytes and each accepted batch
    commit receipt extending it as
    ``d' = SHA256(b"NPBJ26" + d + u64be(n) + R)`` where ``n`` is the new
    ...
    def from_bytes(cls, data: bytes) -> "StreamCommitReceiptFrontier":
        ...
        ``version == 1``, ``sequence`` a non-bool u64, ``end`` a
        lowercase hex string decoding to the canonical non-empty
        :class:`StreamReceiptFrontier` encoding, and
        ``digest``/``mac`` lowercase hex strings decoding to exactly 32
        bytes each. After parsing and field validation the record is
        re-encoded with :meth:`to_bytes` and the result must equal the
        input byte for byte, so formatted JSON, whitespace and any
        non-canonical spelling are rejected too. No MAC is verified here
        — neither the commit frontier MAC nor any MAC of the nested
        ``end`` stream-receipt frontier; pass the record to
...
def _verify_stream_commit_receipt_frontier_macs(
    ...
    """Recompute all eight MAC layers of a parsed stream-commit-receipt
    frontier — the frontier's own ``NPBJ25`` MAC over the canonical
    encoding of its first four fields, and the seven layers of its
    ``end`` stream-receipt frontier exactly as
    :func:`_verify_stream_receipt_frontier_macs` checks them: the
    stream-receipt frontier's own ``NPBJ21`` MAC, the nested
    range-batch receipt frontier's own ``NPBJ17`` MAC, that frontier's
    ``end`` range frontier's own ``NPBJ13`` MAC, the embedded commit
    frontier's own ``NPBJ9`` MAC, that frontier's ``end`` receipt
    frontier's own ``NPBJ5`` MAC, its ``end`` state's ``NPBJ1`` MAC and
    the state's embedded checkpoint table's ``NPBL1`` MAC. All eight
    layers are always recomputed and each compared in constant time
...
class StreamCommitReceiptAuditor:
    """Stateful anti-rollback ledger chaining audited
    :class:`StreamReceiptBatchCommitReceipt` commits into one monotone,
    restartable, sequence-numbered :class:`StreamCommitReceiptFrontier`.
    ...
    eight MAC layers on load, all always recomputed and each compared
    in constant time before any result is consulted: the commit
    frontier's own ``NPBJ25`` MAC over the canonical encoding of its
    first four fields, and the seven layers of its ``end``
    stream-receipt frontier — the stream-receipt frontier's own
    ``NPBJ21`` MAC, the nested range-batch receipt frontier's
    ``NPBJ17`` MAC, the range frontier's own ``NPBJ13`` MAC, the
    embedded commit frontier's ``NPBJ9`` MAC, that frontier's ``end``
    receipt frontier's ``NPBJ5`` MAC, the journal state's own
    ...
    batch verification, returning the frozen
    :class:`StreamReceiptFrontier` at the batch ``end`` — and then
    ...
    the fixed 8-byte big-endian ``n`` and the canonical commit-receipt
    bytes, and the new commit frontier is MAC'd as
    ``HMAC-SHA256(key, b"NPBJ25" + C)``. Replaying the last accepted
    receipt is a violation exactly like an old receipt or an old fork:
    the receipt ``start`` no longer equals the current frontier ``end``,
    so :class:`ValueError` is raised and no state changes. A sequence
    that would overflow u64 or a broken link likewise raises
    :class:`ValueError` and leaves the frontier untouched, a
    wrong-typed argument raises :class:`TypeError`, and concurrent
    audits linearize in lock-acquisition order so an audited commit is
    never lost or rolled back.

    The read-only :attr:`checkpoint` property exports the current
    :class:`StreamCommitReceiptFrontier`, or ``None`` before the first
    successfully audited commit, for the caller to persist and pass
    back to a new auditor after a restart; :attr:`state` exposes the
    very same value.
    """
    ...
    def __init__(self, key: object, *, checkpoint: object = None) -> None:
        ...
        if isinstance(checkpoint, StreamCommitReceiptFrontier):
            ...
            try:
                frontier = StreamCommitReceiptFrontier.from_bytes(
                    checkpoint
                )
            except TypeError as error:
                # The argument had the right kind; a field-shape failure
    ...
    def state(self) -> "Optional[StreamCommitReceiptFrontier]":
        """The current :class:`StreamCommitReceiptFrontier`, or ``None``
        before the first successfully audited commit — the very same
        value :attr:`checkpoint` exposes. The returned object is frozen
        and the property read-only."""
        return self._frontier
    ...
    def audit(
        ...
        """Audit one committed
        :class:`StreamReceiptBatchCommitReceipt` against its
        :class:`StreamReceiptBatch` and advance the ledger.

        ``r`` must be a :class:`StreamReceiptBatchCommitReceipt` or its
        canonical
        :meth:`StreamReceiptBatchCommitReceipt.to_bytes` encoding and
        ``x`` the :class:`StreamReceiptBatch` it attests or its
        canonical :meth:`StreamReceiptBatch.to_bytes` encoding — any
        other type raises :class:`TypeError`; a malformed or
        non-canonical encoding, a MAC or digest mismatch, endpoints
        that do not match the batch, a batch that fails verification, a
        receipt ``start`` that does not equal the current frontier
        ``end`` byte for byte (empty when no commit has happened yet) —
        which also rejects the last accepted receipt replayed, an old
        receipt and an old fork — or a sequence that would overflow
        u64 raises :class:`ValueError`. The pair is re-verified exactly
        as :func:`audit_stream_receipt_batch_commit_receipt` verifies it
        and the receipt ``start`` is matched against the ledger under
        the auditor lock, and the sequence, digest-chain head, end and
        frontier MAC are all advanced in the same atomic step, so a
        failed audit — including an overflow or a broken link —
        changes nothing and concurrent audits are serialized in
        lock-acquisition order. Returns the auditor itself.
        """
        with self._lock:
            record = _coerce_stream_receipt_batch_commit_receipt(r, "r")
            batch_record = _coerce_stream_receipt_batch(x, "x")
