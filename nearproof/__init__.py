...
RangeDecision
/ RangeFrontier / RangeReceipt / SPEED_OF_LIGHT_MPS / TrustRevocation / TrustRevocationList
/ Verifier /
...
def _range_frontier_mac(key: bytes, frontier: "RangeFrontier") -> bytes:
...
class RangeFrontier:
    """A key-MAC'd, digest-chained frontier over the audited
    :class:`RangeReceipt` commit stream.

    ``version`` is always ``1``; ``sequence`` a non-bool unsigned 64-bit
    integer (the number of audited :class:`RangeReceipt` commits); ``end``
    the canonical non-empty
    :meth:`JournalBatchReceiptFrontier.to_bytes` encoding of the commit
    frontier the audited range-receipt chain currently ends at; ``digest``
    exactly 32 bytes — the head of a hash chain with ``d0`` fixed at 32 zero
    bytes and each accepted range receipt extending it as
    ...
    def __post_init__(self) -> None:
        if type(self.version) is not int:
            raise TypeError("range frontier version must be an integer")
        if self.version != 1:
            raise ValueError("range frontier version must be 1")
        if isinstance(self.sequence, bool) or type(self.sequence) is not int:
            raise TypeError(
        ...
        if not isinstance(self.end, bytes):
            raise TypeError("range frontier end must be bytes")
        _require_range_frontier_end(self.end)
        for name in ("digest", "mac"):
            value = getattr(self, name)
            if not isinstance(value, bytes):
                raise TypeError(f"range frontier {name} must be bytes")
            if len(value) != 32:
                raise ValueError(
    ...
    def from_bytes(cls, data: bytes) -> "RangeFrontier":
        ...
        input byte for byte, so formatted JSON, whitespace and any
        non-canonical spelling are rejected too. No MAC is verified here —
        neither the range frontier MAC nor the MACs of the commit frontier
        at ``end``; pass the record to :class:`RangeAuditor` for that.
        """
        if not isinstance(data, bytes):
            raise TypeError("range frontier data must be bytes")
        try:
            outer = json.loads(data)
        except ValueError as error:
            raise ValueError(
        ...
        digest = _parse_bit_map_hex(raw_digest, "range frontier digest")
        if len(digest) != 32:
            raise ValueError(
        ...
        if record.to_bytes() != data:
            raise ValueError("range frontier encoding is not canonical")
        return record
...
def _require_range_frontier_end(value: bytes) -> None:
    ...
    try:
        ...
        raise ValueError(
            "range frontier end must be the canonical"
...
def _verify_range_frontier_macs(
    ...
    commit frontier: that frontier's own ``NPBJ9`` MAC and the three
    layers of its ``end`` receipt frontier: that frontier's own ``NPBJ5``
    MAC, its ``end`` state's ``NPBJ1`` MAC and the state's embedded
    checkpoint table's ``NPBL1`` MAC. All layers are always recomputed and
    each compared in constant time before any result is consulted."""
    commit_frontier = JournalBatchReceiptFrontier.from_bytes(frontier.end)
    ...
    commit_frontier_mac_ok = hmac.compare_digest(
    ...
    if (
        ...
        or not state_mac_ok
        or not end_frontier_mac_ok
        or not commit_frontier_mac_ok
        or not frontier_mac_ok
    ):
        raise ValueError("range frontier mac does not match the key")


class RangeAuditor:
    """Stateful auditor chaining verified :class:`RangeReceipt` commits
    into one monotone, restartable, sequence-numbered
    :class:`RangeFrontier`.

    ``key`` must be non-empty ``bytes`` — a non-bytes value raises
    :class:`TypeError`, an empty value :class:`ValueError`; it is the same
    shared key the receipts, ranges and all their frontiers are MAC'd
    with. ``checkpoint`` is keyword-only: ``None`` (the default) starts
    from sequence zero with no commit at all and the range digest chain
    rooted at ``d0`` (32 zero bytes); otherwise it must be a
    ...
    MAC over the canonical encoding of its first four fields, its ``end``
    commit frontier's own ``NPBJ9`` MAC, and the three layers of that
    commit frontier's ``end`` receipt frontier — the embedded checkpoint
    table's ``NPBL1`` MAC, the journal state's own ``NPBJ1`` MAC and the
    receipt frontier's own ``NPBJ5`` MAC. A malformed encoding or any MAC
    mismatch raises :class:`ValueError`. Across a restart the caller must
    pass the value previously exported at :attr:`frontier`; nothing is
    ...
    range digest, the endpoint equality and the whole range verification
    — and then demands the receipt starts exactly where the range
    frontier currently stands: with no frontier the receipt's ``start``
    must be the empty string, and otherwise it must equal the frontier
    ``end`` byte for byte. Only then does the frontier advance: ``n`` is
    the old sequence plus one, the new ``end`` is the receipt's ``end``,
    ...
    advance are one atomic step: a failed audit raises :class:`ValueError`
    (and a wrong-typed argument :class:`TypeError`) and leaves the
    frontier untouched, and concurrent audits linearize in
    lock-acquisition order so a committed receipt is never lost.
    """
    ...
    @property
    def frontier(self) -> "Optional[RangeFrontier]":
        """The current :class:`RangeFrontier`, or ``None`` before the first
        successfully audited range receipt. The returned object is frozen
        and the property read-only; persist its
        :meth:`RangeFrontier.to_bytes` output and pass it back to a new
        auditor to survive a restart."""
        return self._frontier

    def audit(self, r: object, x: object) -> "RangeAuditor":
        """Audit one committed :class:`RangeReceipt` against its
        ...
        frontier ``end`` (the empty string when no receipt has been
        audited yet) raises :class:`ValueError`. The pair is re-verified
        exactly as :func:`audit_receipt` verifies it and the start matched
        against the frontier under the auditor lock, and the sequence,
        digest-chain head, end and range frontier MAC are all advanced in
        the same atomic step, so a failed audit changes nothing and
        concurrent audits are serialized in lock-acquisition order.
        ...
        with self._lock:
            ...
            # end.
            audit_receipt(receipt, x, self._key)
            current = b"" if self._frontier is None else self._frontier.end
            if receipt.start != current:
                raise ValueError(
