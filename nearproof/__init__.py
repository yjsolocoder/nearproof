...
class StreamCommitReceiptAuditor:
    ...
    def audit_bundle(self, x: object) -> "StreamCommitReceiptAuditor":
        ...
        1. the bundle is parsed per its canonical contract and its
           ...
           time;
        2. the non-empty ``start`` frontier and the ``end`` frontier
           each have all eight MAC layers recomputed exactly as the
           checkpoint load checks them — the commit frontier's own
           ``NPBJ25`` MAC and the seven layers of its ``end``
           stream-receipt frontier — each compared in constant time,
           and no replay begins until every layer of both endpoints
           passes;
        3. the bundle ``start`` must equal the current frontier byte
           for byte: an empty ledger accepts only ``start == b""`` and
           a non-empty ledger demands
        ...
        4. the carried receipts are replayed in order onto a tentative
           ...
           in constant time, its ``start`` must continue byte for byte
           from the tentative frontier ``end`` (the first from the
           bundle start), the u64 sequence must not overflow and the
           digest advances as
           ``SHA256(b"NPBJ26" + d + u64be(n) + R)``;
        ...
        untouched; re-submitting an already committed bundle is
        rejected because its ``start`` no longer matches the advanced
        frontier, and concurrent :meth:`audit`/:meth:`audit_bundle`
        calls linearize in lock-acquisition order so a committed chain
        is never lost or partially applied. Returns the auditor itself.
        """
        with self._lock:
...
def _require_stream_commit_receipt_bundle_frontier(
    ...
    ``value`` is already known to be ``bytes``; when ``allow_empty``
    holds, ``b""`` (the chain starts with no batch commit at all) is
    also accepted. :meth:`StreamCommitReceiptFrontier.from_bytes`
...
class StreamCommitReceiptBundle:
    """A key-MAC'd, transferable bundle sealing one contiguous
    :class:`StreamReceiptBatchCommitReceipt` chain against the commit
    ledger.

    ``version`` is always ``1``. ``start`` is the canonical
    :meth:`StreamCommitReceiptFrontier.to_bytes` encoding of the commit
    frontier the chain starts from, or ``b""`` when the chain starts
    with no batch commit at all (sequence zero, the digest chain rooted
    at ``d0``). ``receipts`` is a non-empty ordered tuple of canonical
    :meth:`StreamReceiptBatchCommitReceipt.to_bytes` bytes — the batch
    commit receipts to replay in order; the bundle carries only the
    receipt bodies, never the attested batches. ``end`` is the
    canonical non-empty :meth:`StreamCommitReceiptFrontier.to_bytes`
    encoding of the commit frontier the chain ends at. ``mac`` is
    exactly 32 bytes — ``HMAC-SHA256(key, b"NPBJ27" + C)`` where ``C``
    is the canonical compact encoding of the first four fields (the
    version, the lowercase-hex start, the array of lowercase-hex
    receipt encodings and the lowercase-hex end, without ``mac``), the
    prefix and ``C`` concatenated directly with no separator or length
    ...
    def from_bytes(cls, data: bytes) -> "StreamCommitReceiptBundle":
        ...
        receipts = tuple(
            _parse_bit_map_hex(
                raw_receipt, "stream commit receipt bundle receipts"
            )
...
def _replay_stream_commit_receipt_bundle(
    ...
    """Replay a bundle's receipt chain onto a tentative commit frontier.
    ...
    the bundle carries only the receipt bodies, so the attested batches
    ...
    digest-chain head, end frontier and commit frontier MAC advance
    exactly as :meth:`StreamCommitReceiptAuditor.audit` advances them.
    The replay touches no shared state: the caller adopts the returned
    tentative frontier only after its own endpoint comparison, and
    every failure raises :class:`ValueError`.
    """
    for encoding in receipt_encodings:
        ...
        if not hmac.compare_digest(
            ...
            raise ValueError(
                "stream receipt batch commit receipt mac does not match"
                " the key"
            )
...
def seal_stream_commit_receipt_bundle(
    receipts: object, key: object, *, checkpoint: object = None
) -> "StreamCommitReceiptBundle":
    """Audit a contiguous :class:`StreamReceiptBatchCommitReceipt` chain
    against the commit ledger and seal it as a bundle.

    ``receipts`` must be a non-empty iterable whose items are each a
    :class:`StreamReceiptBatchCommitReceipt` or its canonical
    :meth:`StreamReceiptBatchCommitReceipt.to_bytes` encoding, and
    ``key`` the non-empty shared ``bytes`` key. ``checkpoint`` is
    keyword-only: ``None`` (the default) starts the chain with no batch
    commit at all — sequence zero, the digest chain rooted at ``d0`` —
    otherwise it must be a :class:`StreamCommitReceiptFrontier` or its
    canonical :meth:`StreamCommitReceiptFrontier.to_bytes` encoding. A
    non-iterable ``receipts``, a wrong-typed item, a non-``bytes``
    ``key`` or a wrong-kind ``checkpoint`` raises :class:`TypeError`; an
    empty sequence, an empty key, a malformed or non-canonical
    encoding, a MAC mismatch, a sequence overflow or a broken chain
    raises :class:`ValueError`. The whole chain is verified first —
    each receipt's ``NPBJ24`` MAC recomputed, its ``start`` continuing
    byte for byte from the running frontier ``end``, the sequence and
    digest chain advancing exactly as
    :class:`StreamCommitReceiptAuditor` advances them, starting from
    the supplied checkpoint — and only on success is the bundle
    produced purely by computation: ``start`` carries the starting
    point (``b""`` when ``checkpoint`` is ``None``, else the checkpoint
    frontier's canonical encoding), ``receipts`` the canonical encoding
    of every receipt in order and ``end`` the canonical encoding of the
    final commit frontier, MAC'd as
    ...
    try:
        ...
        raise TypeError(
            "receipts must be an iterable of"
            " StreamReceiptBatchCommitReceipt instances or their"
            " canonical bytes"
        ) from None
    # The constructor validates the key (non-empty bytes), parses and
    # MAC-checks a supplied checkpoint at all eight layers and rejects a
    # malformed checkpoint with ValueError.
    auditor = StreamCommitReceiptAuditor(key, checkpoint=checkpoint)
    coerced = [
        _coerce_stream_receipt_batch_commit_receipt(item, "receipts items")
        for item in items
    ]
    if not coerced:
    ...
    if checkpoint is None:
        start = b""
    elif isinstance(checkpoint, StreamCommitReceiptFrontier):
        start = checkpoint.to_bytes()
    else:
    ...
    return replace(
        bundle, mac=_stream_commit_receipt_bundle_mac(key, bundle)  # type: ignore[arg-type]
    )
...
def audit_stream_commit_receipt_bundle(
    ...
    then both endpoint commit frontiers are checked at all eight MAC
    layers each — the commit frontier's own ``NPBJ25`` MAC and the
    seven layers of its ``end`` stream-receipt frontier, every layer
    always recomputed and each compared in constant time — and finally
    the carried receipts are replayed one by one, starting from the
    empty ledger when ``start`` is empty and from the carried starting
    frontier otherwise: each receipt's ``NPBJ24`` MAC is recomputed and
    compared in constant time, its ``start`` must continue byte for
    byte from the running frontier ``end`` (the first from the bundle
    start), the sequence and digest chain advance exactly as
    :class:`StreamCommitReceiptAuditor` advances them, and the replayed
    final frontier must equal the bundle's ``end`` byte for byte. The
    bundle carries only the receipt bodies — the attested batches are
    not replayed. Auditing is a pure check: it touches no auditor state
