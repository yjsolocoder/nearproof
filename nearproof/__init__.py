...
# Domain separation prefixes for the stream commit-receipt bundle-receipt
# frontier: the frontier MAC and the per-receipt digest-chain step.
...
def audit_stream_commit_receipt_bundle_receipt(
    ...
    ``receipt`` must be a :class:`StreamCommitReceiptBundleReceipt` or
    its canonical
    :meth:`StreamCommitReceiptBundleReceipt.to_bytes` encoding,
...
def _stream_commit_receipt_bundle_receipt_frontier_content_bytes(
    ...
    """The canonical compact encoding ``C`` of the first four
    bundle-receipt-frontier fields, ``[1, sequence, end, digest]``."""
    return _encode_payload(
        _stream_commit_receipt_bundle_receipt_frontier_content(frontier)
    )


def _stream_commit_receipt_bundle_receipt_frontier_signature(
    key: bytes, frontier: "StreamCommitReceiptBundleReceiptFrontier"
) -> bytes:
    """``S = HMAC-SHA256(key, b"NPBJ29" + C)`` where ``C`` is the
    canonical encoding of the first four fields. The prefix and ``C`` are
    concatenated directly with no separator or length prefix."""
    return hmac.new(
        key,
...
class StreamCommitReceiptBundleReceiptFrontier:
    ...
    canonical non-empty
    :meth:`StreamCommitReceiptFrontier.to_bytes` encoding of the
    batch-commit ledger frontier the audited receipt stream currently
    ends at; ``digest`` exactly 32 bytes — the head of a hash chain with
    ``d0`` fixed at 32 zero bytes and each accepted bundle receipt
    extending it as ``d' = SHA256(b"NPBJ30" + d + u64be(n) + R)`` where
    ``n`` is the new sequence, ``u64be(n)`` its fixed 8-byte big-endian
    encoding and ``R`` the canonical
    :meth:`StreamCommitReceiptBundleReceipt.to_bytes` bytes, every part
    concatenated directly with no separator or length prefix;
    ``signature`` exactly 32 bytes —
    ``S = HMAC-SHA256(key, b"NPBJ29" + C)`` where ``C`` is the canonical
    compact encoding of the first four fields (the version, sequence,
    ...
    def from_bytes(
        ...
        of the nested ``end`` commit frontier; pass the record to
        ...
        if not 0 <= raw_sequence <= 0xFFFFFFFFFFFFFFFF:
            ...
            raw_end,
            "stream commit receipt bundle receipt frontier end",
        )
        ...
        signature = _parse_bit_map_hex(
            raw_signature,
            "stream commit receipt bundle receipt frontier signature",
        )
        if len(signature) != 32:
            raise ValueError(
                "stream commit receipt bundle receipt frontier signature"
...
def _verify_stream_commit_receipt_bundle_receipt_frontier_signatures(
    key: bytes, frontier: "StreamCommitReceiptBundleReceiptFrontier"
) -> None:
    """Recompute all MAC layers of a parsed bundle-receipt frontier —
    the frontier's own ``NPBJ29`` signature over the canonical encoding
    of its first four fields, plus the eight layers of its ``end``
    batch-commit ledger frontier exactly as
    :func:`_verify_stream_commit_receipt_frontier_macs` recomputes them:
    the commit frontier's own ``NPBJ25`` MAC and the seven layers of
    that frontier's ``end`` stream-receipt frontier (its own ``NPBJ21``
    MAC, the nested range-batch receipt frontier's ``NPBJ17`` MAC, the
    range frontier's own ``NPBJ13`` MAC, the embedded commit frontier's
    ``NPBJ9`` MAC, that frontier's ``end`` receipt frontier's ``NPBJ5``
    MAC, the journal state's own ``NPBJ1`` MAC and the embedded
    checkpoint table's ``NPBL1`` MAC). All nine layers are always
    recomputed and each compared in constant time before any result is
    consulted."""
    ...
    try:
        ...
        nested_ok = False
    if not frontier_signature_ok or not nested_ok:
        raise ValueError(
            "stream commit receipt bundle receipt frontier signature"
...
class StreamCommitReceiptBundleReceiptAuditor:
    ...
    ``key`` must be non-empty ``bytes`` — a non-bytes value raises
    :class:`TypeError`, an empty value :class:`ValueError`; it is the
    same shared key the bundle receipts, bundles and frontiers are
    MAC'd with. ``checkpoint`` is keyword-only: ``None`` (the default)
    starts from sequence zero, the empty ledger, with the digest chain
    rooted at ``d0`` (32 zero bytes); otherwise it must be a
    ...
    checked at every MAC layer, all always recomputed and each compared
    in constant time before any result is consulted: the frontier's own
    ``NPBJ29`` signature over the canonical encoding of its first four
    fields, plus the eight layers of its ``end`` batch-commit ledger
    frontier exactly as
    :func:`_verify_stream_commit_receipt_frontier_macs`
    recomputes them. A malformed encoding or any signature/MAC mismatch
    raises :class:`ValueError`. Across a restart the caller must persist
    the value previously exported at :attr:`checkpoint` and pass it
    back; nothing is persisted by the auditor itself, and no alias of
    it is kept beyond the frozen record itself.

    Each :meth:`audit` accepts one
    :class:`StreamCommitReceiptBundleReceipt` as ``receipt`` and the
    :class:`StreamCommitReceiptBundle` it attests as ``bundle`` (either
    as objects or as their canonical bytes) and, under the auditor
    lock, first re-verifies the pair in full exactly like
    ...
    bytes, and the new frontier is signed as
    ``HMAC-SHA256(key, b"NPBJ29" + C)``. Verification and the frontier
    advance are one atomic step: a failed audit raises
    :class:`ValueError` (and a wrong-typed argument :class:`TypeError`)
    and leaves the frontier untouched — replaying a receipt at the same
    sequence with the same digest, presenting a lower-sequence or
    same-sequence-different-digest receipt, an older replayed receipt or
    an old fork all fail the start linkage exactly like a broken chain,
    a credential mismatch, a tampered record, non-canonical bytes or a
    u64 overflow — and concurrent audits linearize in lock-acquisition
    order so an accepted receipt is never lost or rolled back.
    """
    ...
    def __init__(self, key: object, *, checkpoint: object = None) -> None:
        ...
        if isinstance(
            ...
            raise TypeError(
                ...
                " StreamCommitReceiptBundleReceiptFrontier instance, its"
                " canonical bytes, or None"
            )
        _verify_stream_commit_receipt_bundle_receipt_frontier_signatures(
            self._key, frontier
        )
        self._frontier = frontier
    ...
    def checkpoint(
        ...
        receipt (the empty ledger). The returned object is frozen and the
        property read-only; persist its
    ...
    def audit(
        ...
        the :class:`StreamCommitReceiptBundle` it attests and advance
        the ledger.
        ...
        encoding — any other type raises :class:`TypeError`; a
        malformed or non-canonical encoding, a MAC or digest mismatch, a
        wrong or empty key, a tampered record, endpoints that do not
        match the bundle, a bundle that fails verification (a credential
        mismatch), a sequence that would overflow u64, or a receipt
        ``start`` that does not equal the current frontier ``end``
        (empty when the ledger is still empty) raises
        :class:`ValueError`. Replaying the last accepted receipt (same
        sequence, same digest), a lower-sequence receipt, a
        same-sequence receipt with a different digest, an older receipt
        or an old fork all fail that same start-linkage check. The pair
        is re-verified in full exactly as
