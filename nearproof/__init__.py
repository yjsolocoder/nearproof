...
RangeBatchReceipt / RangeBatchReceiptAuditor /
...
# Domain separation prefixes for the range-batch-receipt frontier: its MAC
# and its digest-chain step respectively.
...
def _range_batch_receipt_frontier_next_digest(
    ...
    """One range-batch-receipt digest-chain step:
...
def _require_range_batch_receipt_frontier_end(value: bytes) -> None:
    """Enforce that a range-batch-receipt frontier ``end`` is the canonical
    non-empty :class:`RangeFrontier` encoding. ``value`` is already known to
    be ``bytes``; :meth:`RangeFrontier.from_bytes` enforces the full contract
    including its canonical re-encoding check, and every violation —
    including the field-shape :class:`TypeError` a malformed inner document
    would otherwise surface — raises :class:`ValueError`."""
    try:
...
class RangeBatchReceiptFrontier:
    """A key-MAC'd, digest-chained frontier over the audited
    :class:`RangeBatchReceipt` batch-receipt stream.

    ``version`` is always ``1``; ``sequence`` a non-bool unsigned 64-bit
    integer (the number of audited :class:`RangeBatchReceipt` receipts);
    ``end`` the canonical non-empty :meth:`RangeFrontier.to_bytes` encoding
    of the range frontier the audited batch-receipt chain currently ends at
    (the audited receipt's own ``end``); ``digest`` exactly 32 bytes — the
    head of a hash chain with ``d0`` fixed at 32 zero bytes and each accepted
    batch receipt extending it as
    ``d' = SHA256(b"NPBJ18" + d + u64be(n) + R)`` where ``n`` is the new
    sequence, ``u64be(n)`` its fixed 8-byte big-endian encoding and ``R`` the
    canonical :meth:`RangeBatchReceipt.to_bytes` bytes, every part
    concatenated directly with no separator or length prefix; ``mac``
    exactly 32 bytes — ``HMAC-SHA256(key, b"NPBJ17" + C)`` where ``C`` is the
    canonical compact encoding of the first four fields (the version,
    sequence, lowercase-hex end and lowercase-hex digest, without ``mac``),
    the prefix and ``C`` concatenated directly with no separator or length
    prefix. Instances are frozen, constructed positionally in field order
    ...
    def from_bytes(cls, data: bytes) -> "RangeBatchReceiptFrontier":
        ...
        ``version == 1``, ``sequence`` a non-bool u64, ``end`` a lowercase
        hex string decoding to the canonical non-empty
        :class:`RangeFrontier` encoding, and ``digest``/``mac`` lowercase
        hex strings decoding to exactly 32 bytes each. After parsing and
        ...
        whitespace and any non-canonical spelling are rejected too. No MAC is
        verified here — neither the range-batch-receipt frontier MAC nor the
        MACs of the ``end`` range frontier; pass the record to
        :class:`RangeBatchReceiptAuditor` for that.
        """
        if not isinstance(data, bytes):
            raise TypeError(
        ...
        if len(digest) != 32:
            raise ValueError(
                "range batch receipt frontier digest must decode to exactly"
                " 32 bytes"
            )
...
def _verify_range_batch_receipt_frontier_macs(
    ...
    """Recompute all six MAC layers of a parsed range-batch-receipt
    frontier — its own ``NPBJ17`` MAC over the canonical encoding of its
    first four fields, and the five layers of its ``end`` range frontier
    exactly as :func:`_verify_range_frontier_macs` checks them: the range
    frontier's own ``NPBJ13`` MAC and the four layers of its ``end`` commit
    frontier (the embedded checkpoint table's ``NPBL1`` MAC, the journal
    state's own ``NPBJ1`` MAC, the receipt frontier's own ``NPBJ5`` MAC and
    the commit frontier's own ``NPBJ9`` MAC). All six layers are always
    recomputed and each compared in constant time before any result is
    consulted."""
    frontier_mac_ok = hmac.compare_digest(
...
class RangeBatchReceiptAuditor:
    """Stateful rollback-proof ledger chaining verified
    :class:`RangeBatchReceipt` batch receipts into one monotone,
    restartable, sequence-numbered :class:`RangeBatchReceiptFrontier`.

    ``key`` must be non-empty ``bytes`` — a non-bytes value raises
    :class:`TypeError`, an empty value :class:`ValueError`; it is the same
    shared key the batch receipts, batches, ranges and range frontiers are
    MAC'd with. ``checkpoint`` is keyword-only: ``None`` (the default)
    starts from sequence zero with no batch receipt at all and the digest
    ...
    raises :class:`TypeError`). A supplied frontier is checked at six MAC
    layers, all always recomputed and each compared in constant time before
    any result is consulted: the range-batch-receipt frontier's own
    ``NPBJ17`` MAC over the canonical encoding of its first four fields, and
    the five layers of its ``end`` range frontier — the range frontier's own
    ``NPBJ13`` MAC and the four layers of its ``end`` commit frontier (the
    embedded checkpoint table's ``NPBL1`` MAC, the journal state's own
    ``NPBJ1`` MAC, the receipt frontier's own ``NPBJ5`` MAC and the commit
    frontier's own ``NPBJ9`` MAC). A malformed encoding or any MAC mismatch
    raises :class:`ValueError`. Across a restart the caller must pass the
    value previously exported at :attr:`state`; nothing is persisted by the
    auditor itself.

    Each :meth:`audit` accepts one :class:`RangeBatchReceipt` and the
    :class:`RangeReceiptBatch` it attests (either as objects or as their
    canonical bytes) and first re-verifies the pair exactly like
    :func:`audit_batch_receipt` — the ``NPBJ16`` receipt MAC, the batch
    digest and endpoints and the whole carried batch verification — and
    then, under the auditor lock, demands the receipt starts exactly where
    the frontier currently stands: with no frontier the receipt's ``start``
    must be the empty string, and otherwise it must equal the frontier
    ``end`` byte for byte. Only then does the frontier advance: ``n`` is the
    old sequence plus one, the new ``end`` is the receipt's ``end``, the new
    digest is
    ``SHA256(b"NPBJ18" + d + u64be(n) + R)`` over the previous digest, the
    fixed 8-byte big-endian ``n`` and the canonical receipt bytes, and the
    new frontier is MAC'd as
    ``HMAC-SHA256(key, b"NPBJ17" + C)``. Verification and the frontier
    advance are one atomic step: a failed audit raises
    :class:`ValueError` (and a wrong-typed argument :class:`TypeError`) and
    leaves the frontier untouched, and concurrent audits linearize in
    lock-acquisition order so a verified receipt is never lost.
    """
    ...
    def __init__(self, key: object, *, checkpoint: object = None) -> None:
        ...
        if isinstance(checkpoint, RangeBatchReceiptFrontier):
            ...
            raise TypeError(
                "checkpoint must be a RangeBatchReceiptFrontier instance,"
                " its canonical bytes, or None"
            )
    ...
    def state(self) -> "Optional[RangeBatchReceiptFrontier]":
        """The current :class:`RangeBatchReceiptFrontier`, or ``None`` before
        the first successfully audited batch receipt. The returned object is
        frozen and the property read-only; persist its
        :meth:`RangeBatchReceiptFrontier.to_bytes` output and pass it back to
    ...
    def audit(
        ...
        """Audit one attested :class:`RangeBatchReceipt` against its
        :class:`RangeReceiptBatch` and advance.

        ``receipt`` must be a :class:`RangeBatchReceipt` or its canonical
        :meth:`RangeBatchReceipt.to_bytes` encoding and ``batch`` the
        attested :class:`RangeReceiptBatch` or its canonical
        :meth:`RangeReceiptBatch.to_bytes` encoding — any other type raises
        :class:`TypeError`; a malformed or non-canonical encoding, a MAC or
        digest mismatch, endpoints that do not match the batch, a batch that
        fails verification, a sequence that would overflow u64, or a receipt
        ``start`` that does not equal the current frontier ``end`` (the
        empty string when no batch receipt has happened yet) raises
        :class:`ValueError`. The pair is re-verified exactly as
        :func:`audit_batch_receipt` verifies it and the start matched
        against the frontier under the auditor lock, and the sequence,
        digest-chain head, end frontier and frontier MAC are all advanced in
        the same atomic step, so a failed audit changes nothing and
        concurrent audits are serialized in lock-acquisition order. Returns
        the auditor itself.
        """
        with self._lock:
            # audit_batch_receipt is a pure check: the NPBJ16 receipt MAC,
            # the batch digest, the endpoints and the whole carried batch
            # chain, returning the frozen range frontier at the batch end.
            final = audit_batch_receipt(receipt, batch, self._key)
