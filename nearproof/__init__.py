...
ReceiptStream / SPEED_OF_LIGHT_MPS / SpanReceiptBundle / StreamReceipt /
...
class RangeAuditor:
    ...
    replaced, mints a transferable :class:`SpanBundleReceipt` attesting
    the committed bundle — ``start`` the bundle's ``start``, the bundle
    digest ``SHA256(bundle.to_bytes())`` over the canonical bundle
    encoding, ``end`` the bundle's declared ``end`` and ``signature``
    ``HMAC-SHA256(key, b"NPBJ36" + C)`` over the canonical compact
    encoding of those four fields. A failure raises before anything is
    minted and leaves the frontier exactly where it stood.
    ...
    def audit_bundle(self, x: object) -> "RangeAuditor":
        ...
        receipt is minted — a successful commit advances the frontier only.
        :meth:`audit`, :meth:`audit_batch`, :meth:`audit_batch_record`,
        :meth:`commit_batch`, :meth:`commit_bundle` and this method share
        the auditor lock and linearize together in lock-acquisition order.
        Returns the auditor itself.
        """
        with self._lock:
    ...
    def commit_bundle(self, x: object) -> "SpanBundleReceipt":
        ...
        :class:`SpanBundleReceipt` attesting the committed bundle.

        ``x`` must be a :class:`SpanReceiptBundle` or its canonical
        :meth:`SpanReceiptBundle.to_bytes` encoding — any other type raises
        :class:`TypeError`; every other contract violation (a malformed or
        non-canonical encoding, a wrong key, a ``NPBJ35`` bundle signature
        or MAC mismatch, a MAC mismatch on either endpoint
        :class:`RangeFrontier`, a broken carried link, a forged receipt,
        an old fork, a bundle replayed twice, a final state that does not
        match the replayed chain, or a u64 overflow) raises
        :class:`ValueError`. The verification is exactly
        :meth:`audit_bundle`'s and runs under the same auditor lock shared
        with :meth:`audit`, :meth:`audit_batch`,
        :meth:`audit_batch_record` and :meth:`commit_batch`; the frontier
        is replaced once, and only when the whole bundle verifies, and the
        receipt is then minted over it — ``start`` the bundle's ``start``
        (``b""`` before the first committed receipt), ``bundle_digest``
        ``SHA256(bundle.to_bytes())`` over the canonical bundle encoding
        (binding the receipt to that one bundle and no other bundle over
        the same endpoints), ``end`` the bundle's declared ``end`` and
        ``signature`` ``HMAC-SHA256(key, b"NPBJ36" + C)`` over the
        canonical compact encoding of those four fields. A failure raises
        before anything is minted and leaves the frontier exactly where it
        stood, and concurrent commits linearize in lock-acquisition order.
        Returns the frozen :class:`SpanBundleReceipt`; the auditor itself
        is not returned.
        """
        with self._lock:
            bundle = _coerce_span_receipt_bundle(x, "x")
...
def _span_bundle_receipt_content_bytes(
    ...
    span-bundle-receipt fields, ``[1, S, D, E]``."""
...
class SpanBundleReceipt:
    ...
    bundle and no other bundle over the same endpoints. ``end`` is the
    canonical non-empty :meth:`RangeFrontier.to_bytes` encoding of the
    range frontier the bundle ended at. ``signature`` is exactly 32 bytes
    — ``HMAC-SHA256(key, b"NPBJ36" + C)`` where ``C`` is the canonical
    compact encoding of the first four fields (the version, the
    lowercase-hex start, the lowercase-hex bundle digest and the
    ...
    def to_bytes(self) -> bytes:
        ...
        whitespace, no length prefix."""
        return _encode_payload(
            _span_bundle_receipt_content(self) + [self.signature.hex()]
        )
    ...
    def from_bytes(cls, data: bytes) -> "SpanBundleReceipt":
        ...
        that is empty or decodes to the canonical :class:`RangeFrontier`
        encoding, ``bundle_digest`` and ``signature`` lowercase hex
        strings each decoding to exactly 32 bytes and ``end`` a lowercase
        hex string decoding to the canonical non-empty
        :class:`RangeFrontier` encoding. After parsing and field
        validation the record is re-encoded with :meth:`to_bytes` and the
        result must equal the input byte for byte, so formatted JSON,
        whitespace and any non-canonical spelling are rejected too. No
        signature is verified here — neither the receipt signature nor any
        MAC of the endpoint frontiers; pass the record together with the
        committed bundle to :func:`audit_bundle_receipt` with the shared
        key for that.
        """
        if not isinstance(data, bytes):
        ...
        if raw_version != 1:
            raise ValueError("span bundle receipt version must be 1")
        start = _parse_bit_map_hex(raw_start, "span bundle receipt start")
        _require_span_bundle_receipt_frontier(
            start, "start", allow_empty=True
        )
        bundle_digest = _parse_bit_map_hex(
...
def _coerce_span_bundle_receipt(
    ...
    field-shape failure surfacing while parsing byte content of the right
    kind is a value error."""
    if isinstance(r, SpanBundleReceipt):
        return r
    if isinstance(r, bytes):
        try:
            return SpanBundleReceipt.from_bytes(r)
        except TypeError as error:
            # The argument had the right kind; a field-shape failure
            # surfacing while parsing its byte content is a value error.
            raise ValueError(
...
def audit_bundle_receipt(
    ...
    """Re-verify a :class:`SpanBundleReceipt` against the
    ...
    as ``SHA256(bundle.to_bytes())``; both are always recomputed and each
    compared in constant time before either result is consulted. The
    receipt's ``start`` and ``end`` must then equal the bundle's ``start``
    and ``end`` byte for byte, and finally the bundle itself is verified
    in full exactly as :func:`audit_span_receipt_bundle` verifies it —
    the ``NPBJ35`` bundle signature, every MAC layer of both endpoint
    :class:`RangeFrontier` values and the whole carried receipt chain
    replayed from the bundle ``start`` to its ``end``. Auditing is a pure
    check: it touches no auditor state and returns no partial result — on
    success the frozen :class:`RangeFrontier` the chain ends at is
    returned.
    """
    ...
    if not signature_ok or not digest_ok:
        raise ValueError(
            "span bundle receipt signature or bundle digest does not"
            " match"
        )
    if (
        record.start != bundle_record.start
        or record.end != bundle_record.end
    ):
        raise ValueError(
            "span bundle receipt endpoints do not match the bundle"
        )
