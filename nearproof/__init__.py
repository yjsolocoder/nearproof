...
ReceiptStream / StreamReceipt / SPEED_OF_LIGHT_MPS / TrustRevocation /
TrustRevocationList
/ Verifier /
...
class RangeBatchReceiptAuditor:
    ...
    :meth:`commit_stream` performs exactly the same atomic commitment
    as :meth:`audit_stream` under the same shared lock and, only once
    the live :attr:`state` has advanced, returns a freshly issued
    :class:`StreamReceipt` attesting the committed stream (its start
    and end, ``SHA256(stream.to_bytes())`` and the ``NPBJ20`` MAC over
    ``[1, start, digest, end]``); a failed stream changes no state and
    produces no receipt.
    """
    ...
    def commit_stream(self, x: object) -> "StreamReceipt":
        """Atomically audit and commit one sealed
        :class:`ReceiptStream`, then issue a :class:`StreamReceipt`
        attesting the committed stream.

        ``x`` must be a :class:`ReceiptStream` or its canonical
        :meth:`ReceiptStream.to_bytes` encoding — any other type raises
        :class:`TypeError`; every other contract violation (a malformed
        or non-canonical encoding, a stream MAC mismatch, a start that
        does not equal the current frontier, a carried pair that fails
        its audit, a replayed or broken chain, an end that does not
        match, or a sequence overflow) raises :class:`ValueError`.
        Commitment and issuance are one locked step shared with
        :meth:`audit` and :meth:`audit_stream`: the whole stream is
        committed exactly as :meth:`audit_stream` commits it and the
        :class:`StreamReceipt` is produced only after the live
        :attr:`state` has advanced; on failure the state is untouched
        and no receipt is produced. The returned receipt carries
        ``start`` (the stream start, ``b""`` before the first commit),
        ``end`` (the canonical encoding of the newly current
        frontier), ``digest = SHA256(stream.to_bytes())`` and
        ``mac = HMAC-SHA256(key, b"NPBJ20" + C)`` over ``C``, the
        canonical encoding of ``[1, start, digest, end]`` — the prefix
        and ``C`` concatenated directly with no separator or length
        prefix.
        """
        with self._lock:
            stream = _coerce_receipt_stream(x, "x")
            ...
            # Replay the whole chain on a temporary auditor seeded with
            # the stream start; the live frontier is adopted only when
            # the replayed final frontier equals the stream end.
            candidate = RangeBatchReceiptAuditor(
                self._key,
                checkpoint=stream.start if stream.start else None,
            )
            ...
            self._frontier = final
            return _make_stream_receipt(self._key, stream)


def _require_stream_receipt_frontier(
    value: bytes, name: str, allow_empty: bool
) -> None:
    """Enforce the canonical-:class:`RangeBatchReceiptFrontier`-bytes
    contract of a stream-receipt endpoint.

    ``value`` is already known to be ``bytes``; when ``allow_empty``
    holds, ``b""`` (the committed stream began with no batch commit at
    all) is also accepted.
    :meth:`RangeBatchReceiptFrontier.from_bytes` enforces the full
    contract including its canonical re-encoding check, and every
    violation — including the field-shape :class:`TypeError` a
    malformed inner document would otherwise surface — raises
    :class:`ValueError`."""
    if allow_empty and value == b"":
        return
    try:
        ...
        suffix = " or empty" if allow_empty else ""
        raise ValueError(
            f"stream receipt {name} must be the canonical"
...
def _stream_receipt_content_bytes(receipt: "StreamReceipt") -> bytes:
    """The canonical compact encoding ``C`` of the first four
    stream-receipt fields, ``[1, start, digest, end]``."""
    return _encode_payload(_stream_receipt_content(receipt))


def _stream_receipt_digest(stream: "ReceiptStream") -> bytes:
    """``SHA256(stream.to_bytes())`` over the canonical compact
    receipt-stream encoding."""
    return hashlib.sha256(stream.to_bytes()).digest()


def _stream_receipt_mac(
    key: bytes, receipt: "StreamReceipt"
) -> bytes:
    """``HMAC-SHA256(key, b"NPBJ20" + C)`` where ``C`` is the canonical
    compact encoding of the first four fields ``[1, start, digest,
    end]``. The prefix and ``C`` are concatenated directly with no
...
def _make_stream_receipt(
    ...
    """Issue the :class:`StreamReceipt` attesting a successfully
    committed :class:`ReceiptStream`: ``digest`` over the stream's
    canonical bytes and the ``NPBJ20`` MAC over ``[1, start, digest,
    end]``."""
...
class StreamReceipt:
    ...
    ``version`` is always ``1``. ``start`` is the committed stream's
    ``start`` — ``b""`` when that stream began with no batch commit at
    all, otherwise the canonical
    :meth:`RangeBatchReceiptFrontier.to_bytes` encoding of the frontier
    it started from. ``digest`` is exactly 32 bytes —
    ``SHA256(stream.to_bytes())`` over the stream's canonical compact
    :meth:`ReceiptStream.to_bytes` encoding. ``end`` is the canonical
    non-empty :meth:`RangeBatchReceiptFrontier.to_bytes` encoding of the
    frontier the stream committed to. ``mac`` is exactly 32 bytes —
    ``HMAC-SHA256(key, b"NPBJ20" + C)`` where ``C`` is the canonical
    compact encoding of the first four fields ``[1, start, digest,
    end]`` (the version and the lowercase-hex start, digest and end,
    without ``mac``), the prefix and ``C`` concatenated directly with
    no separator or length prefix. Instances are frozen, constructed
    positionally in field order and compare equal by their fields. A
    field of the wrong type raises :class:`TypeError`; every other
    contract violation raises :class:`ValueError`. No key material is
    ...
    def __post_init__(self) -> None:
        ...
        _require_stream_receipt_frontier(
            self.start, "start", allow_empty=True
        )
        if not isinstance(self.digest, bytes):
    ...
    def from_bytes(cls, data: bytes) -> "StreamReceipt":
        ...
        :meth:`to_bytes` and the result must equal the input byte for
        byte, so formatted JSON, whitespace and any non-canonical
        spelling are rejected too. No MAC or digest is verified here —
        pass the record (with the stream it attests) to
        :func:`audit_stream_receipt` with the shared key for that.
        """
        if not isinstance(data, bytes):
        ...
        _require_stream_receipt_frontier(end, "end", allow_empty=False)
...
def _coerce_stream_receipt(x: object, name: str) -> "StreamReceipt":
    ...
    value error."""
    if isinstance(x, StreamReceipt):
        return x
    if isinstance(x, bytes):
        try:
            return StreamReceipt.from_bytes(x)
        except TypeError as error:
            raise ValueError(
...
def audit_stream_receipt(
    ...
    """Re-verify a :class:`StreamReceipt` against the
    :class:`ReceiptStream` it attests and ``key``.

    ``r`` must be a :class:`StreamReceipt` or its canonical
    :meth:`StreamReceipt.to_bytes` encoding and ``x`` the
    :class:`ReceiptStream` it attests (or its canonical
    :meth:`ReceiptStream.to_bytes` encoding); ``key`` must be non-empty
    ``bytes``. A wrong-typed argument raises :class:`TypeError`; every
    other contract violation raises :class:`ValueError`. All four
    checks run and each is compared in constant time before any result
    is consulted: the receipt ``mac`` recomputed as
    ``HMAC-SHA256(key, b"NPBJ20" + C)`` over the canonical encoding of
    ``[1, start, digest, end]``, the ``digest`` recomputed as
    ``SHA256(x.to_bytes())``, and the ``start`` and ``end`` byte-for-byte
    equal to the stream's; then the stream itself is fully replayed
    exactly as :func:`audit_stream` verifies it (its ``NPBJ19`` MAC and
    the whole carried audit chain). On success the canonical
    :meth:`RangeBatchReceiptFrontier.to_bytes` encoding of the receipt
    ``end`` is returned (``end`` itself, byte for byte).
    """
    receipt = _coerce_stream_receipt(r, "r")
    stream = _coerce_receipt_stream(x, "x")
    if not isinstance(key, bytes):
        raise TypeError("key must be bytes")
    if not key:
        raise ValueError("key must be non-empty")
    # Every layer is always recomputed and compared in constant time
    ...
    if not (mac_ok and digest_ok and start_ok and end_ok):
        raise ValueError(
            "stream receipt does not match the stream and the key"
        )
    # The full stream check: the NPBJ19 MAC and the whole carried
    # receipt/batch chain, returning the canonical end bytes.
    end = audit_stream(stream, key)
    if not hmac.compare_digest(end, receipt.end):
        raise ValueError(
            "stream receipt end does not match the replayed stream"
        )
    return receipt.end
