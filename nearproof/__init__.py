...
# frontier MAC and the receipt digest-chain step respectively.
...
def _stream_receipt_frontier_content_bytes(
    ...
    """The canonical compact encoding ``C`` of the first four
    stream-receipt-frontier fields."""
    return _encode_payload(
        _stream_receipt_frontier_content(frontier)
    )
...
def _require_stream_receipt_frontier_end(value: bytes) -> None:
    ...
    try:
        ...
        raise ValueError(
            "stream receipt frontier end must be the canonical"
            " non-empty RangeBatchReceiptFrontier encoding"
        ) from error


@dataclass(frozen=True)
class StreamReceiptFrontier:
    """A key-MAC'd, digest-chained anti-rollback frontier over audited
    :class:`StreamReceipt` commits.

    ``version`` is always ``1``; ``sequence`` a non-bool unsigned 64-bit
    integer (the number of audited :class:`StreamReceipt` commits);
    ``end`` the canonical non-empty
    :meth:`RangeBatchReceiptFrontier.to_bytes` encoding of the
    range-batch receipt frontier the audited stream-receipt chain
    currently ends at; ``digest`` exactly 32 bytes — the head of a hash
    chain with ``d0`` fixed at 32 zero bytes and each accepted stream
    receipt extending it as
    ``d' = SHA256(b"NPBJ22" + d + u64be(n) + R)`` where ``n`` is the new
    sequence, ``u64be(n)`` its fixed 8-byte big-endian encoding and
    ``R`` the canonical :meth:`StreamReceipt.to_bytes` bytes, every part
    concatenated directly with no separator or length prefix; ``mac``
    exactly 32 bytes — ``M = HMAC-SHA256(key, b"NPBJ21" + C)`` where
    ``C`` is the canonical compact encoding of the first four fields
    (the version, sequence, lowercase-hex end and lowercase-hex digest,
    without ``mac``), the prefix and ``C`` concatenated directly with no
    separator or length prefix. Instances are frozen, constructed
    positionally in field order and compare equal by their fields. A
    field of the wrong type raises :class:`TypeError`; every other
    ...
    def from_bytes(cls, data: bytes) -> "StreamReceiptFrontier":
        ...
        input byte for byte, so formatted JSON, whitespace and any
        non-canonical spelling are rejected too. No MAC is verified here
        — neither the frontier's own ``NPBJ21`` MAC nor any MAC of the
        nested ``end`` range-batch receipt frontier; pass the record to
...
def _verify_stream_receipt_frontier_macs(
    ...
    """Recompute both MAC layers of a stream-receipt checkpoint — the
    frontier's own ``NPBJ21`` MAC over the canonical encoding of its
    first four fields, and every MAC layer of its ``end`` range-batch
    receipt frontier exactly as
    :func:`_verify_range_batch_receipt_frontier_macs` checks them (the
    commit frontier's ``NPBJ17`` MAC and the five layers of its
    ``end`` range frontier). Both layers are always recomputed and each
...
class StreamReceiptAuditor:
    ...
    MAC'd with. ``checkpoint`` is keyword-only: ``None`` (the default)
    starts from sequence zero with no stream-receipt commit at all and
    the digest chain rooted at ``d0`` (32 zero bytes); otherwise it must
    be a :class:`StreamReceiptFrontier` or its canonical
    :meth:`StreamReceiptFrontier.to_bytes` encoding (any other type
    raises :class:`TypeError`). A supplied frontier is checked at two
    MAC layers, both always recomputed and each compared in constant
    time before any result is consulted: the frontier's own ``NPBJ21``
    MAC over the canonical encoding of its first four fields, and every
    MAC layer of its ``end`` range-batch receipt frontier — the commit
    frontier's ``NPBJ17`` MAC and the five layers of its ``end`` range
    frontier. A malformed encoding or any MAC mismatch raises
    :class:`ValueError`. Across a restart the caller must pass the value
    previously exported at :attr:`state`; nothing is persisted by the
    auditor itself.
    ...
    receipt MAC, the receipt digest, the matching endpoints and the
    whole committed-stream replay — and then demands the receipt attest
    exactly the range-batch receipt frontier the ledger currently stands
    at: its ``start`` must equal that frontier's canonical bytes byte for
    byte (and so be empty only before the first stream-receipt commit)
    and its ``end`` is the audited stream's committed end. Only then
    does the frontier advance, atomically with all verification: ``n``
    is the old sequence plus one, the new ``end`` is the receipt's
    ``end``, the new digest is
    ``SHA256(b"NPBJ22" + d + u64be(n) + R)`` over the previous digest,
    the fixed 8-byte big-endian ``n`` and the canonical receipt bytes,
    and the new ledger frontier is MAC'd as
    ``HMAC-SHA256(key, b"NPBJ21" + C)``. A failed audit raises
    :class:`ValueError` (and a wrong-typed argument :class:`TypeError`)
    and leaves the frontier untouched, so an old or forked receipt is
    rejected as a rollback and concurrent audits linearize in
    lock-acquisition order so a committed receipt is never lost.
    """
    ...
    def __init__(self, key: object, *, checkpoint: object = None) -> None:
        ...
        if isinstance(checkpoint, StreamReceiptFrontier):
            ...
            raise TypeError(
                "checkpoint must be a StreamReceiptFrontier"
                " instance, its canonical bytes, or None"
            )
    ...
    def state(self) -> "Optional[StreamReceiptFrontier]":
        """The current :class:`StreamReceiptFrontier`, or ``None``
        before the first successfully audited stream receipt. The
        returned object is frozen and the property read-only; persist
        its :meth:`StreamReceiptFrontier.to_bytes` output and pass it
        back to a new auditor to survive a restart."""
        return self._frontier
    ...
    def audit(
        ...
        """Audit one :class:`StreamReceipt` against its committed
        :class:`ReceiptStream` and advance the anti-rollback ledger.

        ``receipt`` must be a :class:`StreamReceipt` or its canonical
        :meth:`StreamReceipt.to_bytes` encoding and ``stream`` the
        committed :class:`ReceiptStream` or its canonical
        :meth:`ReceiptStream.to_bytes` encoding — any other type raises
        :class:`TypeError`; a malformed or non-canonical encoding, a
        MAC or digest mismatch, endpoints that do not match, a stream
        that fails replay, a receipt whose ``start`` is not exactly the
        ledger's current range-batch receipt frontier (empty only when
        no stream receipt has been committed yet) or whose ``end`` does
        not match the audited stream, or a sequence that would overflow
        u64 raises :class:`ValueError`. The pair is re-verified exactly
        as :func:`audit_stream_receipt` verifies it and the receipt
        start matched against the current frontier under the auditor
        lock, and the sequence, digest-chain head, end and ledger
        frontier MAC are all advanced in the same atomic step, so a
        failed audit changes nothing — an old or forked receipt is
        rejected as a rollback — and concurrent audits are serialized
        in lock-acquisition order. Returns the auditor itself.
        """
        record = _coerce_stream_receipt(receipt, "receipt")
        stream_record = _coerce_receipt_stream(stream, "stream")
        with self._lock:
            # audit_stream_receipt is a pure check: the NPBJ20 receipt
            # MAC, the receipt digest, both endpoints and the whole
            # NPBJ19 committed-stream replay, returning the canonical
            # range-batch receipt frontier bytes at the stream end.
            committed_end = audit_stream_receipt(
                record, stream_record, self._key
            )
            # Anti-rollback: the receipt must start exactly where the
            # ledger currently stands and end exactly where the replayed
            # stream commits.
            current = b"" if self._frontier is None else self._frontier.end
            if record.start != current:
                raise ValueError(
                    "stream receipt start does not match the ledger"
                    " frontier"
                )
            if not hmac.compare_digest(record.end, committed_end):
                raise ValueError(
                    "stream receipt end does not match the audited"
                    " stream"
                )
            previous_sequence = (
                0 if self._frontier is None else self._frontier.sequence
