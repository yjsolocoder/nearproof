...
SpanBundleReceiptAuditor / SpanBundleReceiptFrontier /
...
# Domain separation prefix for the span-bundle-receipt frontier
# signature.
...
class SpanBundleReceiptFrontier:
    ...
    encoding of the range frontier the audited whole-bundle receipt
    chain currently ends at; ``digest`` exactly 32 bytes — the head of
    a hash chain with ``d0`` fixed at 32 zero bytes and each accepted
    receipt extending it as ``d' = SHA256(d + u64be(n) + R)`` where
    ``n`` is the new sequence, ``u64be(n)`` its fixed 8-byte big-endian
    encoding and ``R`` the canonical
    :meth:`SpanBundleReceipt.to_bytes` bytes, every part concatenated
    directly with no separator or length prefix; ``signature`` exactly
    32 bytes — ``HMAC-SHA256(key, b"NPBJ37" + C)`` where ``C`` is the
    canonical compact encoding of the first four fields (the version,
...
def _verify_span_bundle_receipt_frontier_signatures(
    ...
    endpoint's own ``NPBJ13`` MAC and every nested layer below it,
    down through the commit frontier's ``NPBJ9`` MAC). Every layer is
    always recomputed and each compared in constant time before any
    result is consulted."""
    ...
    try:
        ...
        # down through the NPBJ9 journal-batch-receipt frontier MAC and
        # below.
        _verify_range_frontier_macs(
...
class SpanBundleReceiptAuditor:
    ...
    frontier — that endpoint's own ``NPBJ13`` MAC and every nested
    layer below it. A malformed encoding or any signature mismatch
    raises :class:`ValueError`. Across a restart the caller must pass
    the value previously exported at :attr:`checkpoint`; nothing is
    ...
    an older or lower-sequence receipt, presenting a same-sequence
    fork or any other old fork all fail the start linkage exactly like
    a broken chain, a receipt/bundle mismatch or a u64 overflow — and
