...
# Domain separation prefix for the transferable range-receipt batch MAC.
...
def _require_range_receipt_batch_endpoint(
    value: bytes, name: str, allow_empty: bool
) -> None:
    ...
    ``value`` is already known to be ``bytes``; when ``allow_empty`` holds,
    ``b""`` (the batch starts with no range receipt at all) is also
...
def _range_receipt_batch_content(batch: "RangeReceiptBatch") -> list:
    """The JSON-ready first four range-receipt-batch fields (everything
    but ``mac``)."""
    return [
        batch.version,
        batch.start.hex(),
        [[receipt.hex(), record.hex()] for receipt, record in batch.items],
        batch.end.hex(),
    ]


def _range_receipt_batch_content_bytes(batch: "RangeReceiptBatch") -> bytes:
    """The canonical compact encoding ``C`` of the first four
    range-receipt-batch fields."""
...
class RangeReceiptBatch:
    ...
    :class:`RangeReceipt`/:class:`CommitRange` chain against the
    :class:`RangeFrontier`.

    ``version`` is always ``1``. ``start`` is the canonical
    :meth:`RangeFrontier.to_bytes` encoding of the range frontier the chain
    starts from, or ``b""`` when the chain starts with no range receipt at
    all (sequence zero, the range digest chain rooted at ``d0``). ``items``
    is a non-empty ordered tuple of ``(receipt, range)`` pairs — the
    canonical :meth:`RangeReceipt.to_bytes` bytes of each attested receipt
    together with the canonical :meth:`CommitRange.to_bytes` bytes of the
    range it attests, to replay in order. ``end`` is the canonical non-empty
    :meth:`RangeFrontier.to_bytes` encoding of the range frontier the chain
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.start, bytes):
            raise TypeError("range receipt batch start must be bytes")
        _require_range_receipt_batch_endpoint(
            self.start, "start", allow_empty=True
        )
        if not isinstance(self.items, tuple):
            raise TypeError(
        ...
        for item in self.items:
            ...
            if len(item) != 2:
                raise ValueError(
                    "range receipt batch items entries must be (receipt,"
                    " range) pairs"
                )
            receipt, record = item
            if not isinstance(receipt, bytes):
        ...
        if not isinstance(self.end, bytes):
            raise TypeError("range receipt batch end must be bytes")
        _require_range_receipt_batch_endpoint(
            self.end, "end", allow_empty=False
        )
        if not isinstance(self.mac, bytes):
            raise TypeError("range receipt batch mac must be bytes")
        if len(self.mac) != 32:
            raise ValueError(
    ...
    def from_bytes(cls, data: bytes) -> "RangeReceiptBatch":
        ...
        decoding to the canonical non-empty :class:`RangeFrontier` encoding
        ...
        so formatted JSON, whitespace and any non-canonical spelling are
        rejected too. No MAC is verified here — neither the batch MAC nor
        any MAC of the carried receipts, ranges or frontiers; pass the
        record to :func:`audit_range_receipt_batch` with the shared key for
        that.
        """
        if not isinstance(data, bytes):
            raise TypeError("range receipt batch data must be bytes")
        try:
            outer = json.loads(data)
        except ValueError as error:
            raise ValueError(
...
def seal_range_receipt_batch(
    ...
    """Audit a contiguous attested :class:`RangeReceipt` chain against the
    range frontier and seal it as a transferable
    :class:`RangeReceiptBatch`.

    ``items`` must be a non-empty iterable of ``(receipt, range)`` pairs
    whose receipt is each a :class:`RangeReceipt` or its canonical
    :meth:`RangeReceipt.to_bytes` encoding and whose range the attested
    :class:`CommitRange` or its canonical :meth:`CommitRange.to_bytes`
    encoding, and ``key`` the non-empty shared ``bytes`` key.
    ``checkpoint`` is keyword-only: ``None`` (the default) starts the chain
    with no range receipt at all — sequence zero, the range digest chain
    rooted at ``d0`` (the first receipt must then start empty) — otherwise
    it must be a :class:`RangeFrontier` or its canonical
    :meth:`RangeFrontier.to_bytes` encoding. A non-iterable ``items``, a
    non-``bytes`` ``key``, a wrong-typed pair member or a wrong-kind
    ``checkpoint`` raises :class:`TypeError`; an empty sequence, an empty
    key, a malformed or non-canonical encoding, a checkpoint MAC mismatch,
    a failed receipt or range audit, a sequence overflow or a receipt that
    does not continue exactly where the frontier stands raises
    :class:`ValueError`. The whole chain is replayed first, exactly as
    :class:`RangeAuditor` audits it starting from the supplied checkpoint —
    each pair re-verified like :func:`audit_receipt` and the frontier
    advanced per ``NPBJ14`` — and only on success is the batch produced by
    pure computation: its ``start`` carries the starting point (the empty
    string when ``checkpoint`` is ``None``, else the checkpoint's canonical
    encoding), ``items`` every canonical receipt/range pair in order and
    ``end`` the final range frontier's canonical encoding, MAC'd as
    ``HMAC-SHA256(key, b"NPBJ15" + C)`` over the canonical compact encoding
    of those four fields. Sealing touches no auditor state.
    """
    try:
        pairs = list(items)  # type: ignore[arg-type]
    except TypeError:
...
def audit_range_receipt_batch(
    ...
    """Re-verify a :class:`RangeReceiptBatch` against ``key``, from nothing
    or from its carried checkpoint.
    ...
    first; then each non-empty endpoint range frontier is checked at five
    MAC layers — the range frontier's own ``NPBJ13`` MAC and the four
    layers of its ``end`` commit frontier, the embedded checkpoint table's
    ``NPBL1`` MAC, the journal state's own ``NPBJ1`` MAC, the receipt
    frontier's own ``NPBJ5`` MAC and the commit frontier's own ``NPBJ9``
    MAC, all always recomputed and each compared in constant time — and
    only once all of that passes are the carried receipt/range pairs
    replayed one by one, each exactly as :func:`audit_receipt` verifies it
    (the ``NPBJ12`` receipt MAC, the range digest and endpoints and the
    whole carried ``NPBJ11``/``NPBJ10`` range verification) with the range
    frontier advanced per ``NPBJ14`` — the sequence incrementing one u64
    step and the digest chain extending as
    ``SHA256(b"NPBJ14" + d + u64be(n) + R)`` — starting from no range
    receipt at all when ``start`` is empty and from the carried starting
    range frontier otherwise (its five MAC layers recomputed on load). The
    receipts must be contiguous — each one's ``start`` must equal the
    frontier ``end`` currently in force — and the replayed final frontier
    must equal the batch's ``end`` byte for byte. Auditing is a pure
    check: it touches no auditor state and returns no partial result — on
    success the frozen :class:`RangeFrontier` at ``end`` is returned.
    """
    ...
    if not key:
        raise ValueError("key must be non-empty")
    # The batch NPBJ15 MAC is always recomputed and compared in constant
    # time before anything else is consulted.
    if not hmac.compare_digest(
    ...
    # Every non-empty endpoint is checked at five MAC layers — the range
    # frontier's own NPBJ13 MAC and the four layers of its end commit
    # frontier (NPBL1, NPBJ1, NPBJ5, NPBJ9) — before any item is replayed.
    if batch.start:
