...
def _require_span_bundle_receipt_frontier_encoding(
    value: bytes, noun: str, name: str, allow_empty: bool
) -> None:
    """Enforce the canonical-:class:`SpanBundleReceiptFrontier`-bytes
    contract of a span-bundle-receipt batch (``noun`` ``"batch"``) or
    batch-receipt (``noun`` ``"batch receipt"``) endpoint.

    This is the single endpoint validator shared by
    :class:`SpanBundleReceiptBatch`,
    :class:`SpanBundleReceiptBatchReceipt` and
    :class:`SpanBundleReceiptBatchReceiptFrontier`. ``value`` is already
    known to be ``bytes``; when ``allow_empty`` holds, ``b""`` (the chain
    starts with no span-bundle receipt at all) is also accepted.
    ...
    try:
        ...
        suffix = " or empty" if allow_empty else ""
        raise ValueError(
            f"span bundle receipt {noun} {name} must be the canonical"
...
class SpanBundleReceiptBatch:
    ...
    def __post_init__(self) -> None:
        ...
        _require_span_bundle_receipt_frontier_encoding(
            self.start, "batch", "start", allow_empty=True
        )
        if not isinstance(self.receipts, tuple):
            raise TypeError(
        ...
        _require_span_bundle_receipt_frontier_encoding(
            self.end, "batch", "end", allow_empty=False
        )
        if not isinstance(self.signature, bytes):
            raise TypeError(
    ...
    def from_bytes(cls, data: bytes) -> "SpanBundleReceiptBatch":
        ...
        _require_span_bundle_receipt_frontier_encoding(
            start, "batch", "start", allow_empty=True
        )
        if not isinstance(raw_receipts, list):
            raise TypeError(
        ...
        _require_span_bundle_receipt_frontier_encoding(
            end, "batch", "end", allow_empty=False
        )
        signature = _parse_bit_map_hex(
...
class SpanBundleReceiptBatchReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        _require_span_bundle_receipt_frontier_encoding(
            self.start, "batch receipt", "start", allow_empty=True
        )
        if not isinstance(self.batch_digest, bytes):
            raise TypeError(
        ...
        _require_span_bundle_receipt_frontier_encoding(
            self.end, "batch receipt", "end", allow_empty=False
        )
        if not isinstance(self.signature, bytes):
            raise TypeError(
    ...
    def from_bytes(cls, data: bytes) -> "SpanBundleReceiptBatchReceipt":
        ...
        _require_span_bundle_receipt_frontier_encoding(
            start, "batch receipt", "start", allow_empty=True
        )
        batch_digest = _parse_bit_map_hex(
            raw_batch_digest,
        ...
        _require_span_bundle_receipt_frontier_encoding(
            end, "batch receipt", "end", allow_empty=False
        )
        signature = _parse_bit_map_hex(
...
def _require_span_bundle_receipt_batch_receipt_frontier_end(
    ...
    """Enforce that a span-bundle-receipt batch-receipt frontier ``end``
    is the canonical non-empty :class:`SpanBundleReceiptFrontier`
    encoding — the same contract as a non-empty batch-receipt endpoint,
    validated through the one shared endpoint helper. ``value`` is
    already known to be ``bytes``; every violation — including the
    field-shape :class:`TypeError` a malformed inner document would
    otherwise surface — raises :class:`ValueError`."""
    _require_span_bundle_receipt_frontier_encoding(
        value, "batch receipt", "end", allow_empty=False
    )
...
class SpanBundleReceiptBatchReceiptFrontier:
    ...
    :class:`SpanBundleReceiptBatchReceipt` receipts); ``end`` the
    canonical non-empty
    :meth:`SpanBundleReceiptFrontier.to_bytes` encoding carried by the
    last audited batch receipt — the span-bundle-receipt ledger frontier
    the audited commit chain currently ends at; ``digest`` exactly 32
    bytes — the head of a hash chain with ``d0`` fixed at 32 zero bytes
    and each accepted batch receipt extending it as
    ``d' = SHA256(d + u64be(n) + R)`` where ``n`` is the new sequence,
    ``u64be(n)`` its fixed 8-byte big-endian encoding and ``R`` the
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("digest", "signature"):
            ...
            if not isinstance(value, bytes):
                raise TypeError(
                    f"span bundle receipt batch receipt frontier {name}"
                    " must be bytes"
                )
            if len(value) != 32:
                raise ValueError(
    ...
    def from_bytes(
        ...
        of the nested ``end`` receipt frontier; pass the record to
        :class:`SpanBundleReceiptBatchReceiptAuditor` for that.
        """
        if not isinstance(data, bytes):
            raise TypeError(
...
def _verify_span_bundle_receipt_batch_receipt_frontier_signatures(
    ...
    """Recompute the signature layers of a parsed span-bundle-receipt
    batch-receipt frontier — the frontier's own ``NPBJ40`` signature
    over the canonical encoding of its first four fields, and every
    signature layer of its ``end`` span-bundle-receipt frontier exactly
    as :func:`_verify_span_bundle_receipt_frontier_signatures`
    recomputes them (that endpoint's own ``NPBJ37`` signature and every
    MAC layer of its ``end`` range frontier, down through the checkpoint
    table's ``NPBL1`` MAC). Every layer is always recomputed and each
    compared in constant time before any result is consulted."""
    frontier_signature_ok = hmac.compare_digest(
    ...
    try:
        ...
        # endpoint's own NPBJ37 signature and every nested layer of its
        # end range frontier, down through the NPBL1 checkpoint-table
        # MAC.
        _verify_span_bundle_receipt_frontier_signatures(
...
class SpanBundleReceiptBatchReceiptAuditor:
    """Stateful anti-rollback ledger chaining audited
    :class:`SpanBundleReceiptBatchReceipt` commit receipts into one
    monotone, restartable, sequence-numbered
    :class:`SpanBundleReceiptBatchReceiptFrontier`.

    ``key`` must be non-empty ``bytes`` — a non-bytes value raises
    :class:`TypeError`, an empty value :class:`ValueError`; it is the
    same shared key the batch receipts, batches and every nested
    frontier are signed or MAC'd with. ``checkpoint`` is keyword-only:
    ``None`` (the default) starts from sequence zero with no commit
    receipt at all and the digest chain rooted at ``d0`` (32 zero
    bytes); otherwise it must be a
    :class:`SpanBundleReceiptBatchReceiptFrontier` or its canonical
    :meth:`SpanBundleReceiptBatchReceiptFrontier.to_bytes` encoding (any
    other type raises :class:`TypeError`). A supplied frontier is
    ...
    frontier's own ``NPBJ40`` signature over the canonical encoding of
    its first four fields, and every signature layer of its ``end``
    span-bundle-receipt frontier — that endpoint's own ``NPBJ37``
    signature and every MAC layer of the range frontier nested in its
    ``end``, down through the checkpoint table's ``NPBL1`` MAC. A
    malformed encoding or any signature mismatch raises
    :class:`ValueError`. Across a restart the caller must pass the value
    previously exported at :attr:`checkpoint`; nothing is persisted by
    the auditor itself and no alias of the checkpoint is retained
    beyond the frozen frontier object.

    Each :meth:`audit` accepts one
    :class:`SpanBundleReceiptBatchReceipt` and the
    ...
    whole attested batch with its carried receipt chain — and then
    ...
    sequence plus one, the new ``end`` is the receipt's ``end``, the new
    digest is ``SHA256(d + u64be(n) + R)`` over the previous digest,
    the fixed 8-byte big-endian ``n`` and the canonical receipt bytes,
    and the new frontier is signed as
    ``HMAC-SHA256(key, b"NPBJ40" + C)``. Verification, the start gate
    and the frontier advance are one atomic step: a failed audit raises
    :class:`ValueError` (and a wrong-typed argument :class:`TypeError`)
    and leaves the frontier untouched — replaying the last accepted
    receipt (same sequence, same digest), presenting an older or
    lower-sequence receipt, presenting a same-sequence fork or any
    other old fork all fail the start linkage exactly like a broken
    chain, a receipt/batch mismatch or a u64 overflow — and concurrent
    audits linearize in lock-acquisition order so an accepted receipt
    is never lost or rolled back.
    """
    ...
    def __init__(self, key: object, *, checkpoint: object = None) -> None:
        ...
        if checkpoint is None:
            return
        if isinstance(
            checkpoint, SpanBundleReceiptBatchReceiptFrontier
        ):
            frontier = checkpoint
        elif isinstance(checkpoint, bytes):
            try:
            ...
            raise TypeError(
                ...
                " SpanBundleReceiptBatchReceiptFrontier instance, its"
                " canonical bytes, or None"
            )
    ...
    def checkpoint(
        ...
        """The current :class:`SpanBundleReceiptBatchReceiptFrontier`
        ...
        :meth:`SpanBundleReceiptBatchReceiptFrontier.to_bytes` output and
        pass it back to a new auditor to survive a restart."""
        return self._frontier
    ...
    def audit(
        ...
        """Audit one committed
        :class:`SpanBundleReceiptBatchReceipt` against the
        :class:`SpanBundleReceiptBatch` it attests and advance the
        ledger.

        ``receipt`` must be a
        :class:`SpanBundleReceiptBatchReceipt` or its canonical
        :meth:`SpanBundleReceiptBatchReceipt.to_bytes` encoding and
        ``batch`` the attested :class:`SpanBundleReceiptBatch` or its
        canonical :meth:`SpanBundleReceiptBatch.to_bytes` encoding — any
        other type raises :class:`TypeError`; a malformed or
        non-canonical encoding, a signature or digest mismatch, a wrong
        key, endpoints that do not match the batch, a batch that fails
        ...
        older receipt or an old fork all fail that same start-linkage
        check. The pair is re-verified exactly as
        :func:`audit_span_bundle_receipt_batch_receipt` verifies it and
        the start matched against the frontier under the auditor lock,
        and the sequence, digest-chain head, end frontier and frontier
        signature are all advanced in the same atomic step, so a failed
        audit changes nothing and concurrent audits are serialized in
        lock-acquisition order. Returns the auditor itself.
        """
        with self._lock:
            ...
            # endpoints and the whole carried batch chain, returning the
            # frozen SpanBundleReceiptFrontier at the batch end.
            audit_span_bundle_receipt_batch_receipt(
